...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/outputs/v311_recovery_teacher_${STAMP}}"

TASK="${TASK:-Isaac-Forklift-PalletApproach-ToyotaGeoEdgeProgressTeacherV311Recovery-v0}"
DEVICE="${DEVICE:-cuda:0}"
SEED="${SEED:-42}"
NUM_ENVS="${NUM_ENVS:-1024}"
ITERS="${ITERS:-400}"
RUN_NAME="${RUN_NAME:-v311_recovery_teacher_seed${SEED}_${NUM_ENVS}env_${ITERS}iter_${STAMP}}"
...
log() {
  printf '[v311-recovery-teacher] %s\n' "$*"
}
...
if [[ "${RUN_TRAIN}" == "1" ]]; then
  ...
  log "starting v3.11 recovery teacher training: task=${TASK} run_name=${RUN_NAME}"
...
checkpoints=()
append_checkpoint_if_exists checkpoints "${TEACHER_RUN_DIR}/model_250.pt"
append_checkpoint_if_exists checkpoints "${TEACHER_RUN_DIR}/model_300.pt"
append_checkpoint_if_exists checkpoints "${TEACHER_RUN_DIR}/model_350.pt"
append_checkpoint_if_exists checkpoints "${TEACHER_RUN_DIR}/model_399.pt"
...
for checkpoint in "${checkpoints[@]}"; do
  label="v311_recovery_$(basename "${checkpoint}" .pt)"
  eval_cmd=(
