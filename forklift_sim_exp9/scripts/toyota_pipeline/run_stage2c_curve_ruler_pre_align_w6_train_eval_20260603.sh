...
TASK="Isaac-Forklift-PalletApproach-V311LegacyAcceptedTeacherVisualFreshCurveRulerPreAlignW6-v0"
EXPERIMENT_NAME="v311_legacy_visual_curve_guidance"
RUN_NAME="curve_ruler_pre_align_w6_500iter_20260603"

OUTPUT_ROOT="${PROJECT_ROOT}/outputs/accepted_teacher_visual_goal_20260601/v311_legacy_visual_curve_guidance/curve_ruler_pre_align_w6"
...
EVAL_STDOUT="${STDOUT_DIR}/eval_model_499_64ep_novideo_20260603.log"
...
printf '[stage2c-curve-ruler] pid=%s\n' "$$"
printf '[stage2c-curve-ruler] train stdout: %s\n' "${TRAIN_STDOUT}"
printf '[stage2c-curve-ruler] eval stdout: %s\n' "${EVAL_STDOUT}"
printf '[stage2c-curve-ruler] visual summary: %s\n' "${VISUAL_SUMMARY}"
printf '[stage2c-curve-ruler] run_name: %s\n' "${RUN_NAME}"
...
if [[ -z "${run_dir}" ]]; then
  echo "[stage2c-curve-ruler] could not find run dir for ${RUN_NAME}" >&2
  exit 2
fi
...
if [[ -z "${checkpoint}" || ! -f "${checkpoint}" ]]; then
  echo "[stage2c-curve-ruler] no checkpoint found in ${run_dir}" >&2
  exit 3
fi

printf '[stage2c-curve-ruler] eval checkpoint: %s\n' "${checkpoint}"
...
printf '[stage2c-curve-ruler] eval summary: %s/summary.json\n' "${EVAL_OUTPUT}"
