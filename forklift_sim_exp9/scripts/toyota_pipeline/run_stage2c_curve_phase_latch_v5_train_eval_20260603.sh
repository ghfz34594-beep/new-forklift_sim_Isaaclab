...
case "${VARIANT}" in
  v5)
    ...
    ;;
  v5b|dual_gate)
    TASK="Isaac-Forklift-PalletApproach-V311LegacyAcceptedTeacherVisualFreshCurvePhaseLatchPreAlignV5BDualGate-v0"
    RUN_SLUG="curve_phase_latch_pre_align_v5b_dual_gate"
    ;;
  v6|release_gate)
    TASK="Isaac-Forklift-PalletApproach-V311LegacyAcceptedTeacherVisualFreshCurvePhaseLatchPreAlignV6ReleaseGate-v0"
    RUN_SLUG="curve_phase_latch_pre_align_v6_release_gate"
    ;;
  *)
    echo "[stage2c-v5] unknown variant: ${VARIANT}; expected v5, v5b, or v6" >&2
    exit 2
    ;;
esac

RUN_TAG="${RUN_SLUG}${RUN_SUFFIX:+_${RUN_SUFFIX}}"
RUN_NAME="${RUN_TAG}_500iter_20260603"
SMOKE_RUN_NAME="${RUN_TAG}_smoke_2iter_20260603"
OUTPUT_ROOT="${PROJECT_ROOT}/outputs/accepted_teacher_visual_goal_20260601/v311_legacy_visual_curve_guidance/${RUN_TAG}"
...
visual_cmd=(
  ...
  --pallet_visible_min_area_px 250
  --pallet_confident_min_fraction 0.015
...
printf '[stage2c-v5] variant=%s\n' "${VARIANT}"
printf '[stage2c-v5] task=%s\n' "${TASK}"
printf '[stage2c-v5] visual stdout=%s\n' "${VISUAL_STDOUT}"
printf '[stage2c-v5] smoke stdout=%s\n' "${SMOKE_STDOUT}"
printf '[stage2c-v5] train stdout=%s\n' "${TRAIN_STDOUT}"
printf '[stage2c-v5] visual summary=%s\n' "${VISUAL_SUMMARY}"
printf '[stage2c-v5] run_name=%s\n' "${RUN_NAME}"
...
if [[ -z "${run_dir}" ]]; then
  echo "[stage2c-v5] could not find run dir for ${RUN_NAME}" >&2
  exit 3
fi
...
if [[ -z "${checkpoint}" || ! -f "${checkpoint}" ]]; then
  echo "[stage2c-v5] no checkpoint found in ${run_dir}" >&2
  exit 4
fi

printf '[stage2c-v5] eval checkpoint=%s\n' "${checkpoint}"
...
printf '[stage2c-v5] eval summary=%s/summary.json\n' "${EVAL_OUTPUT}"
