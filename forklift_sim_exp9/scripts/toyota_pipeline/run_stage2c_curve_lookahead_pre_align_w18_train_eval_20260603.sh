...
TASK="Isaac-Forklift-PalletApproach-V311LegacyAcceptedTeacherVisualFreshCurveLookaheadPreAlignW18-v0"
EXPERIMENT_NAME="v311_legacy_visual_curve_guidance"
RUN_NAME="curve_lookahead_pre_align_w18_500iter_20260603"

OUTPUT_ROOT="${PROJECT_ROOT}/outputs/accepted_teacher_visual_goal_20260601/v311_legacy_visual_curve_guidance/curve_lookahead_pre_align_w18"
...
printf '[stage2c-curve-lookahead] pid=%s\n' "$$"
printf '[stage2c-curve-lookahead] train stdout: %s\n' "${TRAIN_STDOUT}"
printf '[stage2c-curve-lookahead] eval stdout: %s\n' "${EVAL_STDOUT}"
printf '[stage2c-curve-lookahead] visual summary: %s\n' "${VISUAL_SUMMARY}"
printf '[stage2c-curve-lookahead] run_name: %s\n' "${RUN_NAME}"
...
if [[ -z "${run_dir}" ]]; then
  echo "[stage2c-curve-lookahead] could not find run dir for ${RUN_NAME}" >&2
  exit 2
fi
...
if [[ -z "${checkpoint}" || ! -f "${checkpoint}" ]]; then
  echo "[stage2c-curve-lookahead] no checkpoint found in ${run_dir}" >&2
  exit 3
fi

printf '[stage2c-curve-lookahead] eval checkpoint: %s\n' "${checkpoint}"
...
printf '[stage2c-curve-lookahead] eval summary: %s/summary.json\n' "${EVAL_OUTPUT}"
