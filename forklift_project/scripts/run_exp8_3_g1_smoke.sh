#!/usr/bin/env bash
# Exp8.3 G1 smoke：只改 trajectory terminal geometry package 到 s_success_center。
# 用法：
#   bash scripts/run_exp8_3_g1_smoke.sh
# 行为：
#   - 先退出 conda，避免误用 base/python
#   - 采用与 B0′ smoke 一致的 nohup + 后台启动方式
#   - 日志写入 logs/YYYYMMDD_HHMMSS_train_exp8_3_g1_traj_terminal_s_success_center_smoke.log

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ISAACLAB="${ISAACLAB:-$ROOT/IsaacLab}"
RUN_NAME="exp8_3_g1_traj_terminal_s_success_center_smoke"
LOG_TYPE="train"
...
echo "Started Exp8.3 G1 smoke training."
echo "log: $LOG"
