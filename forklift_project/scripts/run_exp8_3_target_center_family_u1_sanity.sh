#!/usr/bin/env bash
# Exp8.3 runtime U0.5/U1：target-center family 接线 sanity（一次性 headless 检查）
# 用法：
#   bash scripts/run_exp8_3_target_center_family_u1_sanity.sh [b0prime|g2b|g3]
# 默认：b0prime
...
RUN_NAME="exp8_3_target_center_family_u1_${PROFILE}"
...
case "$PROFILE" in
  b0prime)
    ...
    extra_overrides+=(env.exp83_target_center_family_mode=front_center)
    ;;
  g2b)
    extra_overrides+=(env.exp83_traj_goal_mode=front)
...
echo "[INFO] target-center family U1 profile: $PROFILE"
echo "[INFO] log: $LOG"
...
nohup env TERM="$TERM" PYTHONUNBUFFERED=1 CONDA_PREFIX="" CONDA_DEFAULT_ENV="" \
  ...
  env.exp83_runtime_u1_enable=true \
  env.exp83_runtime_u1_fail_fast=true \
  env.exp83_runtime_u1_eps_m=0.001 \
  env.exp83_runtime_u1_probe_margin_m=0.02 \
...
echo "Started Exp8.3 target-center family runtime U1 sanity."
