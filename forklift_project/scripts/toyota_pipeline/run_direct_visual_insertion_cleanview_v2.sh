...
TASK="${TASK:-Isaac-Forklift-PalletApproach-DirectVisualInsertionCleanView-v0}"
RUN_NAME_BASE="${RUN_NAME_BASE:-direct_visual_insertion_cleanview_v2_${STAMP}}"
...
MAX_ITER_MAIN="${MAX_ITER_MAIN:-400}"
...
echo "[direct-visual-v2] task=${TASK}"
echo "[direct-visual-v2] output_root=${OUTPUT_ROOT}"
echo "[direct-visual-v2] main_run_name=${MAIN_RUN_NAME}"
...
if [[ "${RUN_PROBE}" == "1" ]]; then
  if [[ -z "${PROBE_DATASET_DIR}" ]]; then
    echo "[direct-visual-v2] RUN_PROBE=1 requires PROBE_DATASET_DIR" >&2
    exit 2
  fi
...
PY
  ...
  echo "[direct-visual-v2] signed geometry probe skipped; set RUN_PROBE=1 PROBE_DATASET_DIR=... to require it."
fi
...
echo "[direct-visual-v2] starting smoke: ${SMOKE_RUN_NAME}"
{
...
echo "[direct-visual-v2] starting main run: ${MAIN_RUN_NAME}"
{
  printf '%q ' "${RUN_WRAPPER}" -p scripts/reinforcement_learning/rsl_rl/train.py \
    --task "${TASK}" \
    --num_envs "${NUM_ENVS}" \
...
if [[ -z "${MAIN_RUN_DIR}" ]]; then
  echo "[direct-visual-v2] could not find main run dir under ${LOG_ROOT}" >&2
  exit 3
fi

resolve_checkpoint() {
  local requested="$1"
  local fallback_latest="$2"
  if [[ -f "${MAIN_RUN_DIR}/${requested}" ]]; then
    echo "${MAIN_RUN_DIR}/${requested}"
    return 0
  fi
  if [[ "${fallback_latest}" == "1" ]]; then
    find "${MAIN_RUN_DIR}" -maxdepth 1 -type f -name 'model_*.pt' | sort -V | tail -n 1
    return 0
  fi
  echo ""
}

CKPT_200="$(resolve_checkpoint model_200.pt 0)"
if [[ -z "${CKPT_200}" ]]; then
  echo "[direct-visual-v2] model_200.pt not found; evaluating latest <=200 is not implemented, skipping mid checkpoint." >&2
fi
CKPT_FINAL="$(resolve_checkpoint model_400.pt 1)"
if [[ -z "${CKPT_FINAL}" ]]; then
  echo "[direct-visual-v2] no model checkpoint found in ${MAIN_RUN_DIR}" >&2
  exit 4
fi
...
eval_checkpoint() {
  local checkpoint="$1"
  local name="$2"
  local eval_dir="${OUTPUT_ROOT}/eval_${name}"
  echo "[direct-visual-v2] evaluating ${name}: ${checkpoint}"
...
if [[ -n "${CKPT_200}" ]]; then
  eval_checkpoint "${CKPT_200}" "model_200"
fi
eval_checkpoint "${CKPT_FINAL}" "$(basename "${CKPT_FINAL}" .pt)"

echo "[direct-visual-v2] done"
echo "[direct-visual-v2] visual_acceptance=${VIS_SUMMARY}"
echo "[direct-visual-v2] run_dir=${MAIN_RUN_DIR}"
echo "[direct-visual-v2] output_root=${OUTPUT_ROOT}"
