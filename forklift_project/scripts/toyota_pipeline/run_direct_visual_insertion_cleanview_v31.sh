...
TASK="${TASK:-Isaac-Forklift-PalletApproach-DirectVisualInsertionCleanViewV31-v0}"
RUN_NAME_BASE="${RUN_NAME_BASE:-direct_visual_insertion_cleanview_v31_steer_sign_${STAMP}}"
...
MAX_ITER_MAIN="${MAX_ITER_MAIN:-180}"
RUN_EVAL="${RUN_EVAL:-0}"
...
echo "[direct-visual-v31] task=${TASK}"
echo "[direct-visual-v31] output_root=${OUTPUT_ROOT}"
echo "[direct-visual-v31] main_run_name=${MAIN_RUN_NAME}"
...
if [[ -z "${VIS_SUMMARY}" || ! -f "${VIS_SUMMARY}" ]]; then
  echo "[direct-visual-v31] VIS_SUMMARY not provided; running visual acceptance for ${TASK}."
  ...
  echo "[direct-visual-v31] reusing visual acceptance: ${VIS_SUMMARY}"
fi

echo "[direct-visual-v31] starting smoke: ${SMOKE_RUN_NAME}"
{
...
echo "[direct-visual-v31] starting main run: ${MAIN_RUN_NAME}"
{
  printf '%q ' "${RUN_WRAPPER}" -p scripts/reinforcement_learning/rsl_rl/train.py \
    --task "${TASK}" \
    --num_envs "${NUM_ENVS}" \
...
if [[ -z "${MAIN_RUN_DIR}" ]]; then
  echo "[direct-visual-v31] could not find main run dir under ${LOG_ROOT}" >&2
  exit 3
fi

CKPT_FINAL="$(find "${MAIN_RUN_DIR}" -maxdepth 1 -type f -name 'model_*.pt' | sort -V | tail -n 1)"
if [[ -z "${CKPT_FINAL}" ]]; then
  echo "[direct-visual-v31] no model checkpoint found in ${MAIN_RUN_DIR}" >&2
  exit 4
fi

if [[ "${RUN_EVAL}" == "1" ]]; then
  EVAL_DIR="${OUTPUT_ROOT}/eval_$(basename "${CKPT_FINAL}" .pt)"
  echo "[direct-visual-v31] evaluating final checkpoint: ${CKPT_FINAL}"
  ...
  echo "[direct-visual-v31] eval skipped; RUN_EVAL=1 to enable."
fi

echo "[direct-visual-v31] done"
echo "[direct-visual-v31] visual_acceptance=${VIS_SUMMARY}"
echo "[direct-visual-v31] run_dir=${MAIN_RUN_DIR}"
echo "[direct-visual-v31] output_root=${OUTPUT_ROOT}"
