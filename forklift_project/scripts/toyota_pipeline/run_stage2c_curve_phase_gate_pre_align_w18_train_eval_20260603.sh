...
TASK="Isaac-Forklift-PalletApproach-V311LegacyAcceptedTeacherVisualFreshCurvePhaseGatePreAlignW18-v0"
EXPERIMENT_NAME="v311_legacy_visual_curve_guidance"
RUN_NAME="curve_phase_gate_pre_align_w18_500iter_20260603"

OUTPUT_ROOT="${PROJECT_ROOT}/outputs/accepted_teacher_visual_goal_20260601/v311_legacy_visual_curve_guidance/curve_phase_gate_pre_align_w18"
STDOUT_DIR="${OUTPUT_ROOT}/train_stdout"
VISUAL_SUMMARY="$(realpath -m "${OUTPUT_ROOT}/visual_acceptance_train_task/visual_isolation_summary.json")"
...
printf '[stage2c-curve-phase-gate] pid=%s\n' "$$"
printf '[stage2c-curve-phase-gate] train stdout: %s\n' "${TRAIN_STDOUT}"
printf '[stage2c-curve-phase-gate] eval stdout: %s\n' "${EVAL_STDOUT}"
printf '[stage2c-curve-phase-gate] visual summary: %s\n' "${VISUAL_SUMMARY}"
printf '[stage2c-curve-phase-gate] run_name: %s\n' "${RUN_NAME}"
...
if [[ -z "${run_dir}" ]]; then
  echo "[stage2c-curve-phase-gate] could not find run dir for ${RUN_NAME}" >&2
  exit 2
fi
...
if [[ -z "${checkpoint}" || ! -f "${checkpoint}" ]]; then
  echo "[stage2c-curve-phase-gate] no checkpoint found in ${run_dir}" >&2
  exit 3
fi

printf '[stage2c-curve-phase-gate] eval checkpoint: %s\n' "${checkpoint}"
...
printf '[stage2c-curve-phase-gate] eval summary: %s/summary.json\n' "${EVAL_OUTPUT}"
