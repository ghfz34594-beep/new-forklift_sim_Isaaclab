...
RUN_NAME="${RUN_NAME:-exp9_0_no_reference_rewardfix_o2b_seed${SEED}}"
DRY_RUN="${DRY_RUN:-0}"
...
CMD=(
  ...
  # --- O2b: push_free bonus triggers on hold_entry (code changed), weight=3.0 ---
  env.clean_insert_push_free_bonus_enable=true
  env.clean_insert_push_free_bonus_weight=3.0
  # postinsert_align is DISABLED in O2b
...
echo "Started Exp9.0 O2b (hold_entry bonus) experiment."
echo "log: $LOG"
