...
class ExpertConfig:
    ...
    * ``yaw``   : radians (recovered via ``atan2(sin_dyaw, cos_dyaw)``).
    ...
    # ---- Phase-3: Soft corridor guard (reduces 2.0m oscillation) ----
    corridor_guard_band: float = 0.25      # activate guard within pre_insert_dist + band (≈2.30m)
    corridor_guard_ratio: float = 0.85     # tip_lat / safe_corridor ratio threshold to trigger
    corridor_guard_speed: float = 0.35     # max allowed speed when guard fires

    # ---- Phase-3: bbox retreat exit alignment gates ----
    bbox_retreat_exit_lat_ratio: float = 0.80   # tip_lat < safe_corridor * ratio
    ...
    # ---- Phase-3: insertion stall detection ----
    ins_stall_steps: int = 60           # consecutive steps with tiny ins progress → stall
    ins_stall_delta_min: float = 0.005  # |delta insert_norm| below this → no progress
    ins_stall_lat_growth_min: float = 0.02  # lat must also be growing by this per 10 steps
    ins_stall_yaw_growth_min: float = 0.03  # yaw must also be growing (rad) per 10 steps
    ins_stall_backoff_steps: int = 10   # backoff steps triggered on stall
...
class ForkliftExpertPolicy:
    ...
    * **Lift**      : lift after sufficient insertion depth
    ...
    def __init__(
        ...
        # Phase-3: lift latch state
        self._in_lift_latched: bool = False
        self._lift_hold_remaining: int = 0

        # Phase-3: insertion stall detection
        self._ins_stall_counter: int = 0
        self._ins_prev_norm: float = 0.0
        self._ins_prev_lat_abs: float = 0.0
        self._ins_prev_yaw_abs: float = 0.0
    ...
    def reset(self) -> None:
        ...
        # Phase-3: lift latch
        self._in_lift_latched = False
        self._lift_hold_remaining = 0
        # Phase-3: insertion stall
        self._ins_stall_counter = 0
        self._ins_prev_norm = 0.0
        self._ins_prev_lat_abs = 0.0
        self._ins_prev_yaw_abs = 0.0
    ...
    def act(self, obs: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        # ---- Phase-3: Lift latch logic (hysteresis + hold timer) ----
        can_enter_lift = (
            insert_norm >= cfg.lift_on_insert_norm
            and in_insertion
            and abs(lat) < cfg.lift_enter_lat_gate
            and abs(yaw) < cfg.lift_enter_yaw_gate
        )
        if can_enter_lift and not self._in_lift_latched:
            self._in_lift_latched = True
            self._lift_hold_remaining = cfg.lift_hold_steps

        if self._in_lift_latched:
            if self._lift_hold_remaining > 0:
                self._lift_hold_remaining -= 1
            else:
                if insert_norm < cfg.lift_off_insert_norm:
                    self._in_lift_latched = False
        ...
        if self._in_lift_latched:
            ...
            if self._bbox_abort_hold > 0:
                ...
                bbox_tip_lat_limit = min(
                    safe_corridor * cfg.bbox_retreat_exit_lat_ratio,
                    cfg.bbox_retreat_exit_tip_abs_cap,
                )
                bbox_align_ok = (
                    abs(tip_lat) < bbox_tip_lat_limit
                    and abs(lat) < cfg.bbox_retreat_exit_lat_abs
                    and abs(yaw) < cfg.bbox_retreat_exit_yaw_max
                )
                alignment_improved = (dist >= cfg.bbox_retreat_target_dist - 0.005) and bbox_align_ok
        ...
        if stage not in ("lift", "retreat"):
            if in_insertion:
                ...
                ins_delta = insert_norm - self._ins_prev_norm
                if abs(ins_delta) < cfg.ins_stall_delta_min:
                    self._ins_stall_counter += 1
                else:
                    self._ins_stall_counter = 0
                self._ins_prev_norm = insert_norm

                if self._ins_stall_counter > 0 and self._ins_stall_counter % 10 == 0:
                    lat_grew = (abs(lat) - self._ins_prev_lat_abs) >= cfg.ins_stall_lat_growth_min
                    yaw_grew = (abs(yaw) - self._ins_prev_yaw_abs) >= cfg.ins_stall_yaw_growth_min
                    self._ins_prev_lat_abs = abs(lat)
                    self._ins_prev_yaw_abs = abs(yaw)
                    if lat_grew or yaw_grew:
                        self._ins_stall_counter = max(
                            self._ins_stall_counter, cfg.ins_stall_steps - 20
                        )

                if self._ins_stall_counter >= cfg.ins_stall_steps:
                    self._backoff_countdown = max(self._backoff_countdown, cfg.ins_stall_backoff_steps)
                    self._ins_stall_counter = 0
                ...
                # Reset stall counters when leaving insertion
                self._ins_stall_counter = 0
                self._ins_prev_norm = 0.0
                self._ins_prev_lat_abs = 0.0
                self._ins_prev_yaw_abs = 0.0
        ...
        # Phase-3: lift stage overrides rate-limit residual
        if stage == "lift":
            drive = min(drive, cfg.lift_drive_cap)
            steer = 0.0
        ...
        info = {
            ...
            # Phase-3 diagnostics
            "in_lift_latched": self._in_lift_latched,
            "lift_hold_remaining": self._lift_hold_remaining,
            "ins_stall_counter": self._ins_stall_counter,
