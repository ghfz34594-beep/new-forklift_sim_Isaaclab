...
def _spawn_vision_isolation_room(env_prim_path: str, cfg) -> None:
    """Spawn optional static room walls under one env for visual diagnostics."""
    if not bool(getattr(cfg, "vision_room_enable", False)):
        return

    length = float(getattr(cfg, "vision_room_length_m", 10.0))
    width = float(getattr(cfg, "vision_room_width_m", 8.0))
    height = float(getattr(cfg, "vision_room_height_m", 3.0))
    thickness = float(getattr(cfg, "vision_room_wall_thickness_m", 0.15))
...
def _validate_visual_isolation_cfg(cfg) -> None:
    """Fail fast when RGB multi-env isolation relies on impossible clipping."""
    if not bool(getattr(cfg, "visual_isolation_strict_validate", False)):
        return
    if bool(getattr(cfg, "vision_room_enable", False)):
        return
    if not bool(getattr(cfg, "use_camera", False)) or not bool(getattr(cfg, "use_dual_cameras", False)):
        return

    scene_cfg = getattr(cfg, "scene", None)
    num_envs = int(getattr(scene_cfg, "num_envs", 1)) if scene_cfg is not None else 1
    if num_envs <= 1:
        return

    env_spacing = float(getattr(scene_cfg, "env_spacing", 0.0)) if scene_cfg is not None else 0.0
    far_clip = float(getattr(cfg, "dual_camera_far_clip_m", 0.0))
    ratio = float(getattr(cfg, "visual_isolation_far_clip_env_spacing_ratio", 0.45))
    max_far = max(0.0, env_spacing * ratio)
    if env_spacing <= 0.0 or far_clip <= 0.0 or far_clip > max_far:
        raise RuntimeError(
            "dual-camera RGB multi-env isolation requires either vision_room_enable=True "
            "or short far clipping within env spacing: "
            f"num_envs={num_envs}, env_spacing={env_spacing:.3f}, "
            f"dual_camera_far_clip_m={far_clip:.3f}, max_allowed={max_far:.3f}. "
            "Increase scene.env_spacing, reduce dual_camera_far_clip_m, or explicitly enable "
            "vision_room for a diagnostic run."
        )
...
class ForkliftPalletInsertLiftEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: ForkliftPalletInsertLiftEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._dual_camera_enabled = bool(
            getattr(cfg, "use_dual_cameras", False) and self._camera_enabled
        )
        ...
        self._geo_edge_record_cameras = bool(getattr(cfg, "geo_edge_record_cameras", False))
        if self._geo_edge_enabled:
            # 几何观测路径：单一 21D flat tensor，不开 asymmetric critic。
            # teacher-student 采集时允许旁路初始化相机，但 policy obs 仍只返回 21D。
            cfg.observation_space = 21
            if not self._geo_edge_record_cameras:
            ...
            if self._dual_camera_enabled:
                cfg.observation_space = {
                    "image_left": [3, int(cfg.dual_camera_height), int(cfg.dual_camera_width)],
                    "image_right": [3, int(cfg.dual_camera_height), int(cfg.dual_camera_width)],
                    "proprio": int(cfg.toyota_proprio_dim),
                }
            else:
                cfg.observation_space = {
        ...
        self._visual_envelope_termination = torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
        self._last_reset_visual_envelope_termination = torch.zeros(
        ...
        self._prev_center_y_err = torch.zeros((self.num_envs,), device=self.device)
        self._prev_tip_y_err = torch.zeros((self.num_envs,), device=self.device)
        self._prev_tip_axis = torch.zeros((self.num_envs,), device=self.device)
        ...
        self._prev_stage_dist_front = torch.zeros((self.num_envs,), device=self.device)
        ...
        self._ever_inserted_eval = torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
        self._ever_dirty_insert_eval = torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
        self._max_pallet_disp_xy_eval = torch.zeros((self.num_envs,), device=self.device)
        self._pallet_reset_xy_w = torch.zeros((self.num_envs, 2), device=self.device)
        self._debug_reset_x = torch.zeros((self.num_envs,), device=self.device)
        self._debug_reset_y = torch.zeros((self.num_envs,), device=self.device)
        self._debug_reset_yaw_deg = torch.zeros((self.num_envs,), device=self.device)
        
        # 实验 3.2: 近场 commit 状态量
        self._prev_dist_front = torch.zeros((self.num_envs,), device=self.device)
        self._preinsert_recovery_latched = torch.zeros(
            (self.num_envs,), dtype=torch.bool, device=self.device
        )
        self._preinsert_recovery_was_latched = torch.zeros(
            (self.num_envs,), dtype=torch.bool, device=self.device
        )
        ...
        self._preinsert_action_guard_active = torch.zeros(
            (self.num_envs,), dtype=torch.bool, device=self.device
        )
        self._preinsert_action_guard_applied = torch.zeros(
            (self.num_envs,), dtype=torch.bool, device=self.device
        )
        self._preinsert_action_guard_drive_delta = torch.zeros(
            (self.num_envs,), device=self.device
        )
        self._preinsert_action_guard_used = torch.zeros(
            (self.num_envs,), dtype=torch.bool, device=self.device
        )
        self._preinsert_action_guard_steps = torch.zeros(
            (self.num_envs,), dtype=torch.int32, device=self.device
        )
    ...
    def _setup_scene(self):
        ...
        if self._camera_enabled:
            ...
            if self._dual_camera_enabled:
                # Spawn the RTX cameras at the env root and drive their world
                # pose explicitly from the forklift root every render.  Keeping
                # them under Robot/body while also writing world poses can make
                # tiled-camera render products observe a parent/child xform from
                # a different sim update, which breaks the fixed-fork viewpoint.
                ...
                for camera_cfg in (self.cfg.tiled_camera_left, self.cfg.tiled_camera_right):
                    ...
                    camera_cfg.spawn.focal_length = horizontal_aperture / (2.0 * math.tan(hfov_rad / 2.0))

                self._camera_left = TiledCamera(self.cfg.tiled_camera_left)
                self._camera_right = TiledCamera(self.cfg.tiled_camera_right)
            else:
                ...
                self.cfg.tiled_camera.offset.rot = _quat_from_rpy_deg(*self.cfg.camera_rpy_local_deg)
        ...
        _validate_visual_isolation_cfg(self.cfg)
        _spawn_vision_isolation_room("/World/envs/env_0", self.cfg)

        # clone envs
        self.scene.clone_environments(copy_from_source=False)

        # Explicit collision filtering when automatic filtering is unavailable.
        # With replicate_physics=True this is already handled by scene cloning.
        if bool(getattr(self.cfg.scene, "filter_collisions", True)) and not bool(
            getattr(self.cfg.scene, "replicate_physics", True)
        ):
    ...
    def step(self, action: torch.Tensor):
        ...
        self._last_reset_visual_envelope_termination[:] = False
        if len(reset_env_ids) > 0:
            self._last_reset_visual_envelope_termination[reset_env_ids] = self._visual_envelope_termination[
                reset_env_ids
            ]
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        action_noise_std = float(getattr(self.cfg, "toyota_action_noise_std", 0.0))
        if action_noise_std > 0.0:
            clamped_actions = torch.clamp(
                clamped_actions + action_noise_std * torch.randn_like(clamped_actions),
                -1.0,
                1.0,
            )
        ...
        self._apply_preinsert_action_guard()

    def _apply_preinsert_action_guard(self) -> None:
        if not bool(self.cfg.preinsert_action_guard_enable):
            self._preinsert_action_guard_latched[:] = False
            self._preinsert_action_guard_active[:] = False
            self._preinsert_action_guard_applied[:] = False
            self._preinsert_action_guard_drive_delta[:] = 0.0
            self._preinsert_action_guard_steps[:] = 0
            return
        ...
        insert_norm = insert_depth / max(float(self._insert_thresh), 1e-6)
        ...
        if bool(self.cfg.preinsert_action_guard_stateful_enable):
            active = (self._preinsert_action_guard_latched | enter) & (~release)
            self._preinsert_action_guard_latched[:] = active.detach()
        else:
            active = enter
        if bool(self.cfg.preinsert_action_guard_once_per_episode):
            self._preinsert_action_guard_used[:] |= enter.detach()
        if max_guard_steps > 0:
            self._preinsert_action_guard_steps[:] = torch.where(
                active,
                self._preinsert_action_guard_steps + 1,
                torch.zeros_like(self._preinsert_action_guard_steps),
            )

        original_drive = self.actions[:, 0].clone()
        guarded_drive = torch.clamp(
            original_drive,
            max=float(self.cfg.preinsert_action_guard_max_forward_action),
        )
        if bool(self.cfg.preinsert_action_guard_force_reverse):
            reverse_drive = -torch.full_like(
                guarded_drive,
                abs(float(self.cfg.preinsert_action_guard_reverse_action)),
            )
            guarded_drive = torch.minimum(guarded_drive, reverse_drive)
        self.actions[:, 0] = torch.where(active, guarded_drive, original_drive)

        steer_scale = max(0.0, min(1.0, float(self.cfg.preinsert_action_guard_steer_scale)))
        min_abs_steer = max(0.0, min(1.0, float(self.cfg.preinsert_action_guard_min_abs_steer)))
        if bool(self.cfg.preinsert_action_guard_steer_to_reduce_error):
            ...
            self.actions[:, 1] = torch.where(active, steer_cmd, steer)
        elif steer_scale < 1.0 or min_abs_steer > 0.0:
            steer = self.actions[:, 1]
            guarded_steer = steer * steer_scale
            steer_sign = torch.where(
                guarded_steer >= 0.0,
                torch.ones_like(guarded_steer),
                -torch.ones_like(guarded_steer),
            )
            guarded_steer = torch.where(
                torch.abs(guarded_steer) < min_abs_steer,
                steer_sign * min_abs_steer,
                guarded_steer,
            )
            self.actions[:, 1] = torch.where(active, guarded_steer, steer)

        applied = active & (self.actions[:, 0] < original_drive - 1e-6)
        self._preinsert_action_guard_active[:] = active.detach()
        self._preinsert_action_guard_applied[:] = applied.detach()
        self._preinsert_action_guard_drive_delta[:] = (
            original_drive - self.actions[:, 0]
        ).detach()
    ...
    def _normalize_camera_rgb(self, rgb: torch.Tensor, h: int, w: int, label: str) -> torch.Tensor:
        """Normalize camera RGB to (N,3,H,W), float32, [0,1]."""
        # 支持两种常见布局: (N,H,W,3) 或 (N,3,H,W)
        if rgb.ndim != 4:
            raise RuntimeError(f"[camera:{label}] unexpected rgb ndim={rgb.ndim}, expect 4")
        if rgb.shape[-1] == 3:
        ...
        if torch.isnan(rgb).any() or torch.isinf(rgb).any():
            raise RuntimeError(f"[camera:{label}] rgb contains NaN/Inf")

        # shape 保护
        if rgb.shape[2] != h or rgb.shape[3] != w:
            raise RuntimeError(
                f"[camera:{label}] rgb shape mismatch, got {tuple(rgb.shape)}, expect (*,3,{h},{w})"
            )
        return rgb
    ...
    def _get_dual_camera_images(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Return Toyota-style left/right camera images as normalized tensors."""
        h = int(getattr(self.cfg, "dual_camera_height", 224))
        w = int(getattr(self.cfg, "dual_camera_width", 224))
        if (
            not self._camera_initialized
            or self._camera_left is None
    ...
    def _get_toyota_proprio5(self) -> torch.Tensor:
        """Toyota paper proprio: [v_x, v_y, yaw_rate, prev_drive, prev_steer]."""
        root_quat = self.robot.data.root_quat_w
        root_lin_vel = self.robot.data.root_lin_vel_w
        root_ang_vel = self.robot.data.root_ang_vel_w
        yaw = _quat_to_yaw(root_quat)
        ...
        v_xy_r = torch.einsum("nij,nj->ni", R, root_lin_vel[:, :2])
        yaw_rate = root_ang_vel[:, 2:3]
        if float(getattr(self.cfg, "toyota_velocity_obs_noise_std", 0.0)) > 0.0:
            noise_std = float(self.cfg.toyota_velocity_obs_noise_std)
            v_xy_r = v_xy_r + noise_std * torch.randn_like(v_xy_r)
            yaw_rate = yaw_rate + noise_std * torch.randn_like(yaw_rate)
        prev_actions = self.actions[:, :2]
        return torch.cat([v_xy_r, yaw_rate, prev_actions], dim=-1)
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        if self._camera_enabled:
            if self._dual_camera_enabled:
                image_left, image_right = self._get_dual_camera_images()
                proprio = self._get_toyota_proprio5()
    ...
    def _get_potential_approach_rewards(self) -> torch.Tensor:
        """model_1999-style potential reward for Toyota 2D approach.

        This branch keeps the remote 15D baseline's successful stage-potential
        learning surface, but removes lift from the objective.  It is meant for
        a clean geometry teacher before RGB distillation/fine-tuning.
        """
        self._joint_pos[:] = self.robot.root_physx_view.get_dof_positions()
        self._joint_vel[:] = self.robot.root_physx_view.get_dof_velocities()
        ...
        e_band = torch.where(
            stage_dist_front < self.cfg.d1_min,
            self.cfg.d1_min - stage_dist_front,
            torch.where(
                stage_dist_front > self.cfg.d1_max,
                stage_dist_front - self.cfg.d1_max,
                torch.zeros_like(stage_dist_front),
            ),
        )
        E1 = (
            e_band / self.cfg.e_band_scale
        ...
        E2 = (
            stage_dist_front / self.cfg.d2_scale
            + y_err / self.cfg.y_scale2
            + yaw_err_deg / self.cfg.yaw_scale2
        )
        phi2_base = self.cfg.k_phi2 / (1.0 + E2)
        w_band = smoothstep(
            (self.cfg.d1_max - stage_dist_front)
            / max(self.cfg.d1_max - self.cfg.d1_min, 1e-6)
        )
        ...
        w_align2 = torch.exp(
            ...
            - tip_weight * (tip_y_err / self.cfg.tip_y_gate2) ** 2
        )
        phi2 = phi2_base * w_band * w_align2

        w3 = smoothstep((insert_norm - self.cfg.ins_start) / self.cfg.ins_ramp)
        w_align3 = torch.exp(
            -(y_err / self.cfg.y_gate3) ** 2
            - (yaw_err_deg / self.cfg.yaw_gate3) ** 2
            - tip_weight * (tip_y_err / self.cfg.tip_y_gate3) ** 2
        )
        w_lat_gate = torch.exp(-(y_err / self.cfg.ins_lat_gate_sigma) ** 2)
        phi_ins = (
            self.cfg.k_ins
            * (self.cfg.ins_floor + (1.0 - self.cfg.ins_floor) * w_align3)
            * insert_norm
            * w3
            * w_lat_gate
        )
        ...
        if self.cfg.suppress_preinsert_phi_with_w3:
            phi_total = (phi1 + phi2) * (1.0 - w3) + phi_ins
        else:
            phi_total = phi1 + phi2 + phi_ins

        pushfree_curriculum_enabled = bool(
            getattr(self.cfg, "potential_pushfree_curriculum_enable", False)
        )
        if pushfree_curriculum_enabled:
            curriculum_start = float(
                getattr(self.cfg, "potential_pushfree_curriculum_start_step", 0)
            )
            curriculum_ramp = max(
                float(getattr(self.cfg, "potential_pushfree_curriculum_ramp_steps", 1)),
                1.0,
            )
            curriculum_raw = torch.tensor(
                (float(self.common_step_counter) - curriculum_start) / curriculum_ramp,
                device=self.device,
            )
            pushfree_curriculum_progress = smoothstep(curriculum_raw)
            pushfree_curriculum_active = bool(
                float(self.common_step_counter) >= curriculum_start
            )
        else:
            pushfree_curriculum_progress = torch.tensor(0.0, device=self.device)
            pushfree_curriculum_active = False

        r_pot = self.cfg.gamma * phi_total - self._last_phi_total
        ...
        pen_dense = (
            ...
            + self.cfg.rew_action_l2 * (self.actions[:, :2] ** 2).sum(dim=1)
        ...
        pallet_disp_xy = torch.maximum(self._max_pallet_disp_xy_eval, self._pallet_disp_xy())
        self._max_pallet_disp_xy_eval = torch.maximum(
            self._max_pallet_disp_xy_eval,
            pallet_disp_xy.detach(),
        )
        pallet_push_deadband_m = torch.tensor(
            float(self.cfg.pallet_push_deadband_m), device=self.device
        )
        k_pallet_push_pen = torch.tensor(float(self.cfg.k_pallet_push_pen), device=self.device)
        push_quad_weight = torch.tensor(
            float(getattr(self.cfg, "k_pallet_push_pen_quadratic", 0.0)),
            device=self.device,
        )
        full_episode_push_gate_mix = torch.tensor(
            1.0 if bool(getattr(self.cfg, "pallet_push_penalty_full_episode", False)) else 0.0,
            device=self.device,
        )
        insert_progress_push_sigma_m = torch.tensor(
            float(self.cfg.insert_progress_push_sigma_m), device=self.device
        )
        terminal_gate_progress = torch.tensor(
            1.0 if bool(getattr(self.cfg, "push_free_terminal_gate_enable", False)) else 0.0,
            device=self.device,
        )
        if pushfree_curriculum_active:
            progress = pushfree_curriculum_progress
            pallet_push_deadband_m = pallet_push_deadband_m + progress * (
                float(getattr(
                    self.cfg,
                    "potential_pushfree_curriculum_final_pallet_push_deadband_m",
                    self.cfg.pallet_push_deadband_m,
                ))
                - pallet_push_deadband_m
            )
            k_pallet_push_pen = k_pallet_push_pen + progress * (
                float(getattr(
                    self.cfg,
                    "potential_pushfree_curriculum_final_k_pallet_push_pen",
                    self.cfg.k_pallet_push_pen,
                ))
                - k_pallet_push_pen
            )
            push_quad_weight = progress * float(getattr(
                self.cfg,
                "potential_pushfree_curriculum_final_k_pallet_push_pen_quadratic",
                0.0,
            ))
            full_episode_push_gate_mix = progress
            insert_progress_push_sigma_m = insert_progress_push_sigma_m + progress * (
                float(getattr(
                    self.cfg,
                    "potential_pushfree_curriculum_final_insert_progress_push_sigma_m",
                    self.cfg.insert_progress_push_sigma_m,
                ))
                - insert_progress_push_sigma_m
            )
            terminal_gate_progress = progress

        push_excess = torch.clamp(pallet_disp_xy - pallet_push_deadband_m, min=0.0)
        preinsert_push_gate = 1.0 - smoothstep(
            (insert_norm - self.cfg.pallet_push_insert_gate)
            / max(self.cfg.pallet_push_insert_ramp, 1e-6)
        )
        if bool(getattr(self.cfg, "pallet_push_penalty_full_episode", False)):
            push_gate = torch.ones_like(insert_norm)
        else:
            push_gate = preinsert_push_gate
        if pushfree_curriculum_active:
            push_gate = (1.0 - full_episode_push_gate_mix) * preinsert_push_gate + (
                full_episode_push_gate_mix * torch.ones_like(insert_norm)
            )
        pen_pallet_push = -k_pallet_push_pen * push_gate * push_excess
        pen_pallet_push = pen_pallet_push - push_quad_weight * push_gate * (push_excess ** 2)

        if bool(getattr(self.cfg, "use_explicit_progress_reward", False)):
            approach_delta = torch.clamp(
                self._prev_stage_dist_front - stage_dist_front,
                min=0.0,
            )
            insert_delta = torch.clamp(
                insert_norm - self._prev_insert_norm,
                min=0.0,
            )
            approach_align_gate = torch.clamp(
                torch.exp(
                    -(y_err / self.cfg.y_gate2) ** 2
                    - (yaw_err_deg / self.cfg.yaw_gate2) ** 2
                ),
                min=float(self.cfg.approach_progress_align_floor),
                max=1.0,
            )
            approach_active = (
                insert_norm < self.cfg.approach_progress_insert_stop_frac
            ).float()
            r_approach_progress = (
                self.cfg.k_approach_progress
                * approach_delta
                * approach_align_gate
                * approach_active
            )

            insert_align_gate = torch.exp(
                -(center_y_err / self.cfg.insert_progress_center_sigma_m) ** 2
                - (tip_y_err / self.cfg.insert_progress_tip_sigma_m) ** 2
                - (yaw_err_deg / self.cfg.insert_progress_yaw_sigma_deg) ** 2
            )
            insert_push_gate = torch.exp(
                -(
                    pallet_disp_xy
                    / torch.clamp(insert_progress_push_sigma_m, min=1e-6)
                )
                ** 2
            )
            r_insert_progress = (
                self.cfg.k_insert_progress
                * insert_delta
                * insert_align_gate
                * insert_push_gate
            )
        else:
            approach_delta = torch.zeros_like(stage_dist_front)
            insert_delta = torch.zeros_like(insert_norm)
            r_approach_progress = torch.zeros_like(stage_dist_front)
            r_insert_progress = torch.zeros_like(insert_norm)
        ...
        milestone_gate_align = (
            (y_err <= 0.15)
            & (yaw_err_deg <= 8.0)
            & self._milestone_flags[:, 0]
            & (~is_near | tip_align_ok)
        )
        milestone_insert10 = insert_norm >= 0.10
        milestone_insert30 = insert_norm >= 0.30
        milestone_fine_align = (
            (y_err <= 0.10) & (yaw_err_deg <= 5.0) & (~is_near | tip_align_ok)
        )
        milestone_precise_align = (
            (y_err <= 0.05) & (yaw_err_deg <= 3.0) & (~is_near | tip_align_ok)
        )
        milestone_hits = torch.stack(
            [
                milestone_approach,
                milestone_coarse_align,
                milestone_gate_align,
                milestone_insert10,
                milestone_insert30,
                milestone_fine_align,
        ...
        valid_for_flag = milestone_hits & (~dead_zone.unsqueeze(-1))
        ...
        self._fly_counter = torch.where(
            fly_cond, self._fly_counter + 1, torch.zeros_like(self._fly_counter)
        )
        self._early_stop_fly = self._fly_counter >= self.cfg.early_stop_d_xy_steps

        action_abs_max = torch.max(torch.abs(self.actions[:, :2]), dim=1).values
        stall_cond = (
            (~self._is_first_step)
            & (phi_delta_abs < self.cfg.early_stop_stall_phi_eps)
            & (action_abs_max < self.cfg.early_stop_stall_action_eps)
        )
        self._stall_counter = torch.where(
            stall_cond, self._stall_counter + 1, torch.zeros_like(self._stall_counter)
        )
        ...
        align_entry = (center_y_err <= self.cfg.max_lateral_err_m) & (
            yaw_err_deg <= self.cfg.max_yaw_err_deg
        )
        exit_y = self.cfg.max_lateral_err_m * self.cfg.hysteresis_ratio
        exit_yaw = self.cfg.max_yaw_err_deg * self.cfg.hysteresis_ratio
        ...
        insert_entry = insert_depth >= self._insert_thresh
        insert_exit_exceeded = insert_depth < (
            self._insert_thresh - self.cfg.insert_exit_epsilon
        )
        still_ok = insert_entry & align_entry & valid_insert_z
        any_exit_exceeded = align_exit_exceeded | insert_exit_exceeded | (~valid_insert_z)
        ...
        self._hold_counter = torch.where(
            ...
            torch.where(grace_zone, self._hold_counter, decayed),
        )
        geom_success = self._hold_counter >= self._hold_steps
        use_max_push_free_disp = bool(getattr(self.cfg, "push_free_training_use_max_disp", False))
        if pushfree_curriculum_active:
            use_max_push_free_disp = True
        episode_push_free = (
            self._max_pallet_disp_xy_eval < self.cfg.push_free_disp_thresh_m
            if use_max_push_free_disp
            else pallet_disp_xy < self.cfg.push_free_disp_thresh_m
        )
        pushfree_terminal_gate_active = (
            bool(getattr(self.cfg, "push_free_terminal_gate_enable", False))
            or pushfree_curriculum_active
        )
        if pushfree_terminal_gate_active:
            clean_success = geom_success & episode_push_free
            dirty_success = geom_success & (~episode_push_free)
            reward_success = clean_success
        else:
            clean_success = geom_success
            reward_success = geom_success
            dirty_success = torch.zeros_like(geom_success, dtype=torch.bool)
        success = geom_success
        ...
        r_terminal = torch.where(
            clean_success,
            self.cfg.rew_success
            + terminal_gate_progress * float(getattr(self.cfg, "push_free_terminal_bonus", 0.0))
            + time_bonus,
            r_terminal,
        )
        dirty_success_reward_scale = float(
            getattr(self.cfg, "dirty_success_reward_scale", 0.0)
        )
        dirty_success_reward = (
            (1.0 - terminal_gate_progress) * (self.cfg.rew_success + time_bonus)
            + terminal_gate_progress
            * (
                dirty_success_reward_scale * (self.cfg.rew_success + time_bonus)
                - float(getattr(self.cfg, "dirty_success_terminal_penalty", 0.0))
            )
        )
        dirty_terminal_penalty = torch.where(
            dirty_success,
            dirty_success_reward,
            torch.zeros_like(r_terminal),
        )
        r_terminal = r_terminal + dirty_terminal_penalty
        r_terminal = torch.where(
        ...
        phi_align = (
            torch.exp(-(center_y_err / self.cfg.hold_align_sigma_y) ** 2)
            * torch.exp(-(yaw_err_deg / self.cfg.hold_align_sigma_yaw) ** 2)
        )
        ...
        pen_dead_zone = (
            -self.cfg.k_dead_zone
            * dead_zone.float()
            * (insert_norm - self.cfg.dead_zone_insert_thresh)
            * (y_err - self.cfg.dead_zone_lat_thresh)
        )
        pen_dead_zone = torch.clamp(
            pen_dead_zone, min=-self.cfg.dead_zone_pen_clamp, max=0.0
        )
        ...
        self._prev_stage_dist_front = stage_dist_front.detach()
        ...
        self.extras["log"]["potential_reward/phi1"] = phi1.mean()
        self.extras["log"]["potential_reward/phi2"] = phi2.mean()
        self.extras["log"]["potential_reward/phi_ins"] = phi_ins.mean()
        self.extras["log"]["potential_reward/phi_total"] = phi_total.mean()
        self.extras["log"]["potential_reward/r_pot"] = r_pot.mean()
        self.extras["log"]["potential_reward/r_approach_progress"] = (
            r_approach_progress.mean()
        )
        self.extras["log"]["potential_reward/r_insert_progress"] = (
            r_insert_progress.mean()
        )
        self.extras["log"]["potential_reward/pen_dense"] = pen_dense.mean()
        self.extras["log"]["potential_reward/r_terminal"] = r_terminal.mean()
        self.extras["log"]["potential_reward/r_milestone"] = milestone_reward.mean()
        self.extras["log"]["potential_reward/r_hold_align"] = r_hold_align.mean()
        self.extras["log"]["potential_reward/pen_tip_proximity"] = pen_tip_proximity.mean()
        self.extras["log"]["potential_reward/pen_pallet_push"] = pen_pallet_push.mean()
        self.extras["log"]["potential_reward/push_gate"] = push_gate.mean()
        self.extras["log"]["potential_reward/pushfree_curriculum_progress"] = (
            pushfree_curriculum_progress
        )
        self.extras["log"]["potential_reward/terminal_gate_progress"] = (
            terminal_gate_progress
        )
        self.extras["log"]["potential_reward/dirty_terminal_penalty"] = (
            dirty_terminal_penalty.mean()
        )
        self.extras["log"]["potential_reward/pen_dead_zone"] = pen_dead_zone.mean()
        self.extras["log"]["potential_reward/pen_early_stop"] = early_stop_penalty.mean()
        self.extras["log"]["potential_reward/w_band"] = w_band.mean()
        self.extras["log"]["potential_reward/w3"] = w3.mean()
        self.extras["log"]["potential_reward/w_align2"] = w_align2.mean()
        self.extras["log"]["potential_reward/w_align3"] = w_align3.mean()
        self.extras["log"]["diag/approach_delta_mean"] = approach_delta.mean()
        self.extras["log"]["diag/insert_delta_mean"] = insert_delta.mean()
        ...
        self.extras["log"]["err/center_lateral_mean"] = center_y_err.mean()
        ...
        self.extras["log"]["diag/eval_max_pallet_disp_xy_mean"] = (
            self._max_pallet_disp_xy_eval.mean()
        )
        self.extras["log"]["diag/z_err_mean"] = z_err.mean()
        self.extras["log"]["diag/max_hold_counter"] = self._hold_counter.max()
        ...
        self.extras["log"]["phase/frac_geom_success"] = geom_success.float().mean()
        self.extras["log"]["phase/frac_reward_success"] = reward_success.float().mean()
        self.extras["log"]["phase/frac_dirty_success"] = dirty_success.float().mean()
        self.extras["log"]["phase/frac_episode_push_free"] = episode_push_free.float().mean()
        self.extras["log"]["phase/frac_inserted_push_free"] = (
            inserted_push_free.float().mean()
        )
        ...
        if reset_mask.any():
            ...
            batch_rate = num_successes / max(num_resets, 1)
    ...
    def _get_progress_teacher_rewards(self) -> torch.Tensor:
        """Fresh privileged-geometry teacher reward.

        This branch deliberately does not follow the old potential/checkpoint
        route.  It uses privileged forklift/pallet geometry to reward measurable
        progress: get closer, reduce alignment errors, insert deeper, then hold
        the inserted pose.  Lift, RGB, push-free hard gates, and action guards
        are kept out of this teacher.
        """
        self._joint_pos[:] = self.robot.root_physx_view.get_dof_positions()
        self._joint_vel[:] = self.robot.root_physx_view.get_dof_velocities()
        ...
        rel_base = root_pos[:, :2] - pallet_pos[:, :2]
        y_signed = torch.sum(rel_base * v_lat, dim=-1)
        y_err = torch.abs(y_signed)
        ...
        s_base = torch.sum(rel_base_axis * u_in, dim=-1)
        dist_front_tip = torch.clamp(s_front - s_tip, min=0.0)
        dist_front_base = torch.clamp(s_front - s_base, min=0.0)
        # Progress-teacher training must not reward pushing after the forks have
        # already reached the pallet face.  Use the fork-tip distance as the
        # teacher progress variable even if older baselines used base distance.
        stage_dist_front = dist_front_tip
        ...
        insert_depth = torch.clamp(s_tip - s_front, min=0.0)
        insert_depth = torch.where(valid_insert_z, insert_depth, torch.zeros_like(insert_depth))
        insert_norm = torch.clamp(insert_depth / max(float(self._insert_thresh), 1e-6), 0.0, 1.5)
        self._last_insert_depth = insert_depth.detach()
        ...
        dist_delta = torch.clamp(self._prev_stage_dist_front - stage_dist_front, min=0.0)
        dist_increase = torch.clamp(stage_dist_front - self._prev_stage_dist_front, min=0.0)
        y_delta = torch.clamp(self._prev_y_err - y_err, min=0.0)
        center_y_delta = torch.clamp(self._prev_center_y_err - center_y_err, min=0.0)
        tip_y_delta = torch.clamp(self._prev_tip_y_err - tip_y_err, min=0.0)
        tip_axis_delta = torch.clamp(s_tip - self._prev_tip_axis, min=0.0)
        yaw_delta = torch.clamp(self._prev_yaw_err_deg - yaw_err_deg, min=0.0)
        insert_delta = torch.clamp(insert_norm - self._prev_insert_norm, min=0.0)

        near_gate = smoothstep(
            (float(getattr(self.cfg, "progress_teacher_near_dist_m", 1.0)) - stage_dist_front)
            / max(float(getattr(self.cfg, "progress_teacher_near_ramp_m", 0.5)), 1e-6)
        )
        mouth_gate = smoothstep(
            (float(getattr(self.cfg, "progress_teacher_commit_dist_m", 0.45)) - stage_dist_front)
            / max(float(getattr(self.cfg, "progress_teacher_commit_dist_m", 0.45)), 1e-6)
        )
        approach_active = (insert_norm < float(getattr(self.cfg, "progress_teacher_approach_stop_insert_norm", 0.10))).float()
        align_gate = torch.exp(
            -(center_y_err / max(float(getattr(self.cfg, "progress_teacher_center_gate_m", 0.28)), 1e-6)) ** 2
            - (yaw_err_deg / max(float(getattr(self.cfg, "progress_teacher_yaw_gate_deg", 12.0)), 1e-6)) ** 2
        )
        insert_gate_floor = float(getattr(self.cfg, "progress_teacher_insert_gate_floor", 0.0))
        insert_gate = torch.clamp(insert_gate_floor + (1.0 - insert_gate_floor) * align_gate, max=1.0)
        episode_pallet_disp_xy = self._max_pallet_disp_xy_eval
        if bool(getattr(self.cfg, "progress_teacher_pushfree_curriculum_enable", False)):
            curriculum_start = float(getattr(self.cfg, "progress_teacher_pushfree_curriculum_start_step", 0))
            curriculum_ramp = max(
                float(getattr(self.cfg, "progress_teacher_pushfree_curriculum_ramp_steps", 1)),
                1.0,
            )
            curriculum_raw = torch.tensor(
                (float(self.common_step_counter) - curriculum_start) / curriculum_ramp,
        ...
        push_sigma_m = (
            ...
            + (
                float(getattr(self.cfg, "progress_teacher_push_sigma_m", 0.25))
                - float(getattr(self.cfg, "progress_teacher_push_sigma_start_m", 0.14))
            )
            * pushfree_curriculum
        )
        clean_disp_m = (
            float(getattr(self.cfg, "progress_teacher_clean_disp_start_m", 0.12))
            + (
                float(getattr(self.cfg, "progress_teacher_clean_disp_m", 0.08))
                - float(getattr(self.cfg, "progress_teacher_clean_disp_start_m", 0.12))
            )
            * pushfree_curriculum
        )
        success_disp_m = (
            float(getattr(self.cfg, "progress_teacher_success_disp_start_m", 0.10))
            + (
                float(getattr(self.cfg, "progress_teacher_success_disp_m", 0.10))
                - float(getattr(self.cfg, "progress_teacher_success_disp_start_m", 0.10))
            )
            * pushfree_curriculum
        )
        dirty_insert_weight = (
            float(getattr(self.cfg, "progress_teacher_dirty_insert_weight_start", 0.0))
            + (
                float(getattr(self.cfg, "progress_teacher_dirty_insert_penalty_weight", 0.0))
                - float(getattr(self.cfg, "progress_teacher_dirty_insert_weight_start", 0.0))
            )
            * pushfree_curriculum
        )
        dirty_insert_disp_m = (
            float(getattr(self.cfg, "progress_teacher_dirty_insert_disp_start_m", 0.08))
            + (
                float(getattr(self.cfg, "progress_teacher_dirty_insert_disp_m", 0.05))
                - float(getattr(self.cfg, "progress_teacher_dirty_insert_disp_start_m", 0.08))
            )
            * pushfree_curriculum
        )
        push_penalty_weight = (
            float(getattr(self.cfg, "progress_teacher_push_penalty_start_weight", 0.4))
            + (
                float(getattr(self.cfg, "progress_teacher_push_penalty", 0.4))
                - float(getattr(self.cfg, "progress_teacher_push_penalty_start_weight", 0.4))
            )
            * pushfree_curriculum
        )
        push_deadband_m = (
            float(getattr(self.cfg, "progress_teacher_push_deadband_start_m", 0.08))
            + (
                float(getattr(self.cfg, "progress_teacher_push_deadband_m", 0.08))
                - float(getattr(self.cfg, "progress_teacher_push_deadband_start_m", 0.08))
            )
            * pushfree_curriculum
        )
        preinsert_termination_m = (
            float(getattr(self.cfg, "progress_teacher_preinsert_termination_start_m", 0.18))
            + (
                float(getattr(self.cfg, "preinsert_push_termination_m", 0.12))
                - float(getattr(self.cfg, "progress_teacher_preinsert_termination_start_m", 0.18))
            )
            * pushfree_curriculum
        )
        dirty_termination_m = (
            float(getattr(self.cfg, "progress_teacher_dirty_termination_start_m", 0.22))
            + (
                float(getattr(self.cfg, "dirty_push_termination_m", 0.18))
                - float(getattr(self.cfg, "progress_teacher_dirty_termination_start_m", 0.22))
            )
            * pushfree_curriculum
        )
        reward_gate_disp_xy = torch.where(
            torch.full(
                (self.num_envs,),
                bool(getattr(self.cfg, "progress_teacher_clean_gate_use_max_disp", False)),
                dtype=torch.bool,
                device=self.device,
            ),
            episode_pallet_disp_xy,
            pallet_disp_xy,
        )
        push_gate = torch.exp(
            -(
                reward_gate_disp_xy
                / torch.clamp(push_sigma_m, min=1e-6)
            )
            ** 2
        )
        clean_gate = (reward_gate_disp_xy <= clean_disp_m).float()
        commit_gate = mouth_gate * torch.exp(
            -(
                center_y_err
                / max(float(getattr(self.cfg, "progress_teacher_commit_center_sigma_m", 0.18)), 1e-6)
            )
            ** 2
            - (
                tip_y_err
                / max(float(getattr(self.cfg, "progress_teacher_commit_tip_sigma_m", 0.16)), 1e-6)
            )
            ** 2
            - (
                yaw_err_deg
                / max(float(getattr(self.cfg, "progress_teacher_commit_yaw_sigma_deg", 10.0)), 1e-6)
            )
            ** 2
        ) * clean_gate
        ...
        r_away = (
            -float(getattr(self.cfg, "progress_teacher_away_penalty_weight", 0.0))
            * dist_increase
            * approach_active
        )
        r_align = near_gate * (
            float(getattr(self.cfg, "progress_teacher_center_weight", 8.0)) * y_delta
            + float(getattr(self.cfg, "progress_teacher_yaw_weight", 0.25)) * yaw_delta
        )
        r_fork_align = near_gate * approach_active * (
            float(getattr(self.cfg, "progress_teacher_fork_center_weight", 0.0)) * center_y_delta
            + float(getattr(self.cfg, "progress_teacher_fork_tip_weight", 0.0)) * tip_y_delta
        )
        r_insert = (
            float(getattr(self.cfg, "progress_teacher_insert_weight", 55.0))
            * insert_delta
            * insert_gate
            * push_gate
            * clean_gate
        ...
        r_commit_progress = (
            float(getattr(self.cfg, "progress_teacher_commit_progress_weight", 120.0))
            * tip_axis_delta
            * commit_gate
            * push_gate
            * commit_active
        ...
        r_commit_forward = (
            float(getattr(self.cfg, "progress_teacher_commit_forward_weight", 0.20))
            * forward_action
            * commit_gate
            * push_gate
            * commit_active
        ...
        r_aligned_approach_progress = (
            float(getattr(self.cfg, "progress_teacher_aligned_approach_progress_weight", 0.0))
            * dist_delta
            * near_gate
            * align_gate
            * push_gate
        ...
        mouth_stall_penalty = (
            -float(getattr(self.cfg, "progress_teacher_mouth_stall_penalty", 0.06))
            * commit_gate
            * not_committing
            * commit_active
        )
        ...
        align_potential = (
            float(getattr(self.cfg, "progress_teacher_align_potential_weight", 0.15))
            * torch.exp(
                -(center_y_err / max(float(getattr(self.cfg, "progress_teacher_center_sigma_m", 0.30)), 1e-6)) ** 2
                - (yaw_err_deg / max(float(getattr(self.cfg, "progress_teacher_yaw_sigma_deg", 12.0)), 1e-6)) ** 2
                - (tip_y_err / max(float(getattr(self.cfg, "progress_teacher_tip_sigma_m", 0.25)), 1e-6)) ** 2
            )
            * (1.0 - torch.clamp(mouth_gate * align_gate, 0.0, 1.0))
        )
        insert_potential = (
            float(getattr(self.cfg, "progress_teacher_insert_potential_weight", 1.0))
            * torch.clamp(insert_norm, 0.0, 1.0)
            * insert_gate
            * push_gate
            * clean_gate
        )
        ...
        if bool(getattr(self.cfg, "progress_teacher_close_noinsert_gate_enable", False)):
            ...
            close_noinsert_gate = close_gate * no_insert_gate * approach_active * clean_gate
            positive_floor = max(
                0.0,
        ...
        push_excess = torch.clamp(
            episode_pallet_disp_xy - push_deadband_m,
            min=0.0,
        )
        push_penalty = -push_penalty_weight * push_excess
        dirty_disp_xy = torch.where(
            torch.full(
                (self.num_envs,),
                bool(getattr(self.cfg, "progress_teacher_dirty_insert_use_max_disp", True)),
                dtype=torch.bool,
                device=self.device,
            ),
            episode_pallet_disp_xy,
        ...
        dirty_insert_excess = torch.clamp(
            dirty_disp_xy - dirty_insert_disp_m,
            min=0.0,
        )
        dirty_insert_active = smoothstep(
            (
                insert_norm
                - float(getattr(self.cfg, "progress_teacher_dirty_insert_min_norm", 0.15))
            )
            / max(
                1.0 - float(getattr(self.cfg, "progress_teacher_dirty_insert_min_norm", 0.15)),
                1e-6,
            )
        )
        ...
        if bool(getattr(self.cfg, "progress_teacher_misaligned_forward_penalty_enable", False)):
            near_forward_zone = smoothstep(
                (
                    float(getattr(self.cfg, "progress_teacher_misaligned_forward_near_m", 0.70))
                    - stage_dist_front
                )
                / max(float(getattr(self.cfg, "progress_teacher_misaligned_forward_near_m", 0.70)), 1e-6)
            )
            center_excess = torch.clamp(
                center_y_err - float(getattr(self.cfg, "progress_teacher_misaligned_forward_center_m", 0.14)),
                min=0.0,
            )
            tip_excess = torch.clamp(
                tip_y_err - float(getattr(self.cfg, "progress_teacher_misaligned_forward_tip_m", 0.14)),
                min=0.0,
            )
            yaw_excess = torch.clamp(
                yaw_err_deg - float(getattr(self.cfg, "progress_teacher_misaligned_forward_yaw_deg", 7.0)),
                min=0.0,
            ) / 10.0
            misalignment_score = torch.clamp(center_excess + tip_excess + yaw_excess, max=1.0)
            misaligned_forward_penalty = (
                -float(getattr(self.cfg, "progress_teacher_misaligned_forward_penalty_weight", 1.0))
                * forward_action
                * near_forward_zone
                * approach_active
                * misalignment_score
        ...
        if bool(getattr(self.cfg, "progress_teacher_steer_sign_reward_enable", False)):
            ...
            steer_sign_gate = signal_gate * near_sign_gate * preinsert_sign_gate * push_gate * clean_gate
        ...
        if bool(getattr(self.cfg, "preinsert_push_termination_enable", False)):
            preinsert_push_mask = (
                (episode_pallet_disp_xy >= preinsert_termination_m)
                & (insert_depth < self._insert_thresh)
                & (self.episode_length_buf >= int(getattr(self.cfg, "preinsert_push_termination_min_steps", 8)))
            )
        else:
            preinsert_push_mask = torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
        if bool(getattr(self.cfg, "dirty_push_termination_enable", False)):
            dirty_push_mask = (
                (episode_pallet_disp_xy >= dirty_termination_m)
                & (self.episode_length_buf >= int(getattr(self.cfg, "dirty_push_termination_min_steps", 8)))
            )
        else:
            dirty_push_mask = torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
        r_preinsert_push_termination = -float(
            getattr(self.cfg, "preinsert_push_termination_penalty_weight", 80.0)
        ) * preinsert_push_mask.float()
        r_dirty_push_termination = -float(
            getattr(self.cfg, "dirty_push_termination_penalty_weight", 120.0)
        ) * dirty_push_mask.float()
        visual_envelope_mask = (
            self._visual_envelope_termination
            if bool(getattr(self.cfg, "visual_envelope_termination_enable", False))
            else torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
        )
        r_visual_envelope_termination = -float(
            getattr(self.cfg, "visual_envelope_termination_penalty_weight", 60.0)
        ) * visual_envelope_mask.float()
        ...
        success_disp_xy = torch.where(
            torch.full(
                (self.num_envs,),
                bool(getattr(self.cfg, "progress_teacher_success_use_max_disp", True)),
                dtype=torch.bool,
                device=self.device,
            ),
            episode_pallet_disp_xy,
        ...
        clean_ok = success_disp_xy <= success_disp_m
        dirty_insert_event = (
            (success_disp_xy > float(getattr(self.cfg, "progress_teacher_dirty_insert_termination_disp_m", 0.05)))
            & (
                insert_norm
                >= float(getattr(self.cfg, "progress_teacher_dirty_insert_termination_min_norm", 0.80))
            )
            & (self.episode_length_buf >= int(getattr(self.cfg, "progress_teacher_dirty_insert_termination_min_steps", 8)))
        )
        if not bool(getattr(self.cfg, "progress_teacher_dirty_insert_termination_enable", False)):
            dirty_insert_event = torch.zeros_like(dirty_insert_event)
        self._progress_teacher_dirty_insert_termination = dirty_insert_event
        r_dirty_insert_event_termination = -float(
            getattr(self.cfg, "progress_teacher_dirty_insert_termination_penalty_weight", 90.0)
        ) * dirty_insert_event.float()
        still_ok = insert_entry & align_entry & valid_insert_z & clean_ok
        any_exit_exceeded = align_exit_exceeded | insert_exit_exceeded | (~valid_insert_z)
        grace_zone = (~still_ok) & (~any_exit_exceeded)
        hold_counter_prev = self._hold_counter.clone()
        decayed = self._hold_counter * self.cfg.hold_counter_decay
        self._hold_counter = torch.where(
            still_ok,
            self._hold_counter + 1,
        ...
        hold_delta = torch.clamp(
            (self._hold_counter - hold_counter_prev) / float(self._hold_steps),
            min=0.0,
            max=1.0,
        )
        ...
        time_out_now = self.episode_length_buf >= self.max_episode_length - 1
        r_terminal = torch.zeros((self.num_envs,), device=self.device)
        r_terminal = torch.where(success, self.cfg.rew_success + time_bonus, r_terminal)
        r_terminal = torch.where(
            time_out_now & ~success,
            r_terminal + self.cfg.rew_timeout,
            r_terminal,
        ...
        if "log" not in self.extras:
            self.extras["log"] = {}
        self.extras["log"]["progress_teacher/r_approach"] = r_approach.mean()
        self.extras["log"]["progress_teacher/r_away"] = r_away.mean()
        self.extras["log"]["progress_teacher/r_align"] = r_align.mean()
        self.extras["log"]["progress_teacher/r_fork_align"] = r_fork_align.mean()
        self.extras["log"]["progress_teacher/r_insert"] = r_insert.mean()
        self.extras["log"]["progress_teacher/r_commit_progress"] = r_commit_progress.mean()
        ...
        self.extras["log"]["progress_teacher/visual_envelope_term_penalty"] = (
            r_visual_envelope_termination.mean()
        )
        self.extras["log"]["progress_teacher/dist_delta"] = dist_delta.mean()
        self.extras["log"]["progress_teacher/dist_increase"] = dist_increase.mean()
        self.extras["log"]["progress_teacher/y_delta"] = y_delta.mean()
        self.extras["log"]["progress_teacher/center_y_delta"] = center_y_delta.mean()
        self.extras["log"]["progress_teacher/tip_y_delta"] = tip_y_delta.mean()
        self.extras["log"]["progress_teacher/tip_axis_delta"] = tip_axis_delta.mean()
        self.extras["log"]["progress_teacher/yaw_delta"] = yaw_delta.mean()
        self.extras["log"]["progress_teacher/insert_delta"] = insert_delta.mean()
        self.extras["log"]["progress_teacher/align_gate"] = align_gate.mean()
        self.extras["log"]["progress_teacher/insert_gate"] = insert_gate.mean()
        self.extras["log"]["progress_teacher/mouth_gate"] = mouth_gate.mean()
        self.extras["log"]["progress_teacher/commit_gate"] = commit_gate.mean()
        self.extras["log"]["progress_teacher/push_gate"] = push_gate.mean()
        self.extras["log"]["progress_teacher/clean_gate"] = clean_gate.mean()
        self.extras["log"]["progress_teacher/pushfree_curriculum"] = pushfree_curriculum
        self.extras["log"]["progress_teacher/curr_push_sigma_m"] = push_sigma_m
        self.extras["log"]["progress_teacher/curr_clean_disp_m"] = clean_disp_m
        self.extras["log"]["progress_teacher/curr_success_disp_m"] = success_disp_m
        self.extras["log"]["progress_teacher/curr_dirty_weight"] = dirty_insert_weight
        self.extras["log"]["progress_teacher/curr_dirty_disp_m"] = dirty_insert_disp_m
        self.extras["log"]["progress_teacher/curr_push_deadband_m"] = push_deadband_m
        self.extras["log"]["progress_teacher/curr_preinsert_term_m"] = preinsert_termination_m
        self.extras["log"]["progress_teacher/curr_dirty_term_m"] = dirty_termination_m
        self.extras["log"]["progress_teacher/reward_gate_disp_xy"] = reward_gate_disp_xy.mean()
        ...
        self.extras["log"]["err/insert_norm_mean"] = insert_norm.mean()
        self.extras["log"]["diag/pallet_disp_xy_mean"] = pallet_disp_xy.mean()
        self.extras["log"]["diag/eval_max_pallet_disp_xy_mean"] = self._max_pallet_disp_xy_eval.mean()
        self.extras["log"]["diag/z_err_mean"] = z_err.mean()
        self.extras["log"]["diag/max_hold_counter"] = self._hold_counter.max()
        self.extras["log"]["diag/max_hold_counter_frac"] = (
            self._hold_counter.max() / float(self._hold_steps)
        ...
        self.extras["log"]["diag/visual_envelope_termination_frac"] = visual_envelope_mask.float().mean()
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if bool(getattr(self.cfg, "use_progress_teacher_reward", False)):
            return self._get_progress_teacher_rewards()
        if bool(getattr(self.cfg, "use_potential_approach_reward", False)):
            return self._get_potential_approach_rewards()
        ...
        rel_tip = tip[:, :2] - pallet_pos[:, :2]
        s_tip = torch.sum(rel_tip * u_in, dim=-1)
        rel_fc_axis = fork_center[:, :2] - pallet_pos[:, :2]
        s_center_axis = torch.sum(rel_fc_axis * u_in, dim=-1)
        s_root_axis = torch.sum(rel_robot * u_in, dim=-1)
        s_front = -0.5 * self.cfg.pallet_depth_m

        dist_front = torch.clamp(s_front - s_tip, min=0.0)
        dist_front_center = torch.clamp(s_front - s_center_axis, min=0.0)
        dist_front_root = torch.clamp(s_front - s_root_axis, min=0.0)
        if self.cfg.preinsert_recovery_dist_ref == "tip":
            preinsert_recovery_dist = dist_front
        elif self.cfg.preinsert_recovery_dist_ref == "fork_center":
            preinsert_recovery_dist = dist_front_center
        elif self.cfg.preinsert_recovery_dist_ref == "root":
            preinsert_recovery_dist = dist_front_root
        else:
            raise ValueError(
                f"Unsupported preinsert_recovery_dist_ref: {self.cfg.preinsert_recovery_dist_ref}"
            )
        if self.cfg.preinsert_recovery_trigger_dist_ref == "same":
            preinsert_recovery_trigger_dist = preinsert_recovery_dist
        elif self.cfg.preinsert_recovery_trigger_dist_ref == "tip":
            preinsert_recovery_trigger_dist = dist_front
        elif self.cfg.preinsert_recovery_trigger_dist_ref == "fork_center":
            preinsert_recovery_trigger_dist = dist_front_center
        elif self.cfg.preinsert_recovery_trigger_dist_ref == "root":
            preinsert_recovery_trigger_dist = dist_front_root
        else:
            raise ValueError(
                "Unsupported preinsert_recovery_trigger_dist_ref: "
                f"{self.cfg.preinsert_recovery_trigger_dist_ref}"
            )
        insert_depth = torch.clamp(s_tip - s_front, min=0.0)
        ...
        push_free_train = push_free
        if self.cfg.push_free_training_use_max_disp:
            push_free_train = (
                self._max_pallet_disp_xy_eval
                < self.cfg.push_free_disp_thresh_m
            )
        eval_dirty_disp = (
            self._max_pallet_disp_xy_eval
            > self.cfg.push_free_disp_thresh_m + self.cfg.eval_dirty_insert_disp_margin_m
        )
        eval_inserted_now = hold_state.insert_entry
        eval_dirty_insert_now = eval_inserted_now & eval_dirty_disp
        eval_dirty_insert_new = eval_dirty_insert_now & (~self._ever_dirty_insert_eval)
        self._ever_inserted_eval |= eval_inserted_now.detach()
        self._ever_dirty_insert_eval |= eval_dirty_insert_now.detach()
        inserted_push_free_reward = hold_state.hold_entry & push_free_train  # O2b: was (insert_depth >= self._insert_thresh)
        if self.cfg.align_before_contact_disable_insert_bonus:
            inserted_push_free_reward = torch.zeros_like(inserted_push_free_reward)
        if self.cfg.clean_insert_unlock_enable:
            clean_insert_unlocked = (
                (center_y_err <= self.cfg.clean_insert_unlock_align_gate_m)
                & (tip_y_err <= self.cfg.clean_insert_unlock_tip_gate_m)
                & (yaw_err_deg <= self.cfg.clean_insert_unlock_yaw_gate_deg)
                & (pallet_disp_xy <= self.cfg.clean_insert_unlock_push_gate_m)
            )
        else:
        ...
        if self.cfg.clean_insert_unlock_enable:
            training_success_next = training_success_next & clean_insert_unlocked
        if self.cfg.push_free_training_success_enable:
            success_next = training_success_next & push_free_train
        else:
        ...
        if self.cfg.align_before_contact_enable:
            align_center_gate = torch.exp(
                -(center_y_err / max(self.cfg.align_before_contact_center_sigma_m, 1e-6)) ** 2
            )
            align_yaw_gate = torch.exp(
                -(yaw_err_deg / max(self.cfg.align_before_contact_yaw_sigma_deg, 1e-6)) ** 2
            )
            align_tip_gate = torch.where(
                dist_front <= self.cfg.tip_align_near_dist,
                torch.exp(
                    -(tip_y_err / max(self.cfg.align_before_contact_tip_sigma_m, 1e-6)) ** 2
                ),
                torch.ones_like(tip_y_err),
            )
            align_contact_score = align_center_gate * align_yaw_gate * align_tip_gate
            align_before_contact_reward = (
                self.cfg.align_before_contact_reward_weight
                * preinsert_contact_active
                * push_free.float()
                * align_contact_score
            )
            align_before_contact_hold_reward = (
                self.cfg.align_before_contact_hold_weight
                * preinsert_contact_active
                * push_free.float()
                * (center_y_err <= self.cfg.clean_insert_center_sigma_m).float()
                * (tip_y_err <= self.cfg.tip_align_entry_m).float()
                * (yaw_err_deg <= self.cfg.clean_insert_yaw_sigma_deg).float()
            )
        else:
        ...
        if self.cfg.clean_insert_unlock_enable:
            insert_reward_gate = 1.0 - clean_insert_progress + clean_insert_progress * unlock_gate
            r_d = r_d * insert_reward_gate
            r_cd = r_cd * insert_reward_gate
            r_cpsi = r_cpsi * insert_reward_gate
        if self.cfg.eval_dirty_max_reward_gate_enable:
            eval_dirty_max_gate = torch.where(
                push_free_train,
                torch.ones_like(insert_norm),
                torch.full_like(insert_norm, self.cfg.eval_dirty_max_reward_gate_floor),
            )
            r_d = r_d * eval_dirty_max_gate
            r_cd = r_cd * eval_dirty_max_gate
            r_cpsi = r_cpsi * eval_dirty_max_gate
        else:
            eval_dirty_max_gate = torch.ones_like(insert_norm)

        preinsert_recovery_insert_gate = (
            insert_norm < self.cfg.preinsert_insert_frac_max
        ).float()
        preinsert_recovery_narrow_mask_bool = torch.zeros_like(insert_norm, dtype=torch.bool)
        if self.cfg.preinsert_recovery_enable:
            preinsert_recovery_narrow_mask_bool = (
                (dist_front < self.cfg.preinsert_recovery_narrow_dist_m)
                & (
                    (center_y_err >= self.cfg.preinsert_recovery_narrow_center_m)
                    | (tip_y_err >= self.cfg.preinsert_recovery_narrow_tip_m)
                    | (yaw_err_deg >= self.cfg.preinsert_recovery_narrow_yaw_deg)
                )
                & (preinsert_recovery_insert_gate > 0.0)
            )
            if self.cfg.preinsert_recovery_narrow_initial_near_hard_only:
                preinsert_recovery_narrow_mask_bool = (
                    preinsert_recovery_narrow_mask_bool
                    & self._debug_near_hard_curriculum.bool()
                )
            preinsert_recovery_misaligned = (
                (center_y_err >= self.cfg.preinsert_recovery_center_trigger_m)
                | (yaw_err_deg >= self.cfg.preinsert_recovery_yaw_trigger_deg)
            )
            preinsert_recovery_enter_gate_bool = (
                preinsert_recovery_narrow_mask_bool
                if self.cfg.preinsert_recovery_narrow_near_hard_enable
                else torch.ones_like(preinsert_recovery_misaligned, dtype=torch.bool)
            )
            if self.cfg.preinsert_recovery_stateful_enable:
                center_norm = center_y_err / max(self.cfg.preinsert_recovery_center_trigger_m, 1e-6)
                yaw_norm = yaw_err_deg / max(self.cfg.preinsert_recovery_yaw_trigger_deg, 1e-6)
                misalign_norm = torch.clamp(torch.maximum(center_norm, yaw_norm), min=0.0, max=1.0)
                preinsert_recovery_target_dist = torch.clamp(
                    self.cfg.preinsert_recovery_target_base_m
                    + self.cfg.preinsert_recovery_target_extra_m * misalign_norm,
                    max=self.cfg.preinsert_recovery_target_max_m,
                )
                target_low = torch.clamp(
                    preinsert_recovery_target_dist - self.cfg.preinsert_recovery_target_low_tol_m,
                    min=0.0,
                )
                target_high = (
                    preinsert_recovery_target_dist
                    + self.cfg.preinsert_recovery_target_high_tol_m
                )
                preinsert_recovery_release_ready_bool = (
                    (preinsert_recovery_dist >= target_low)
                    & (center_y_err <= self.cfg.preinsert_recovery_release_center_m)
                    & (yaw_err_deg <= self.cfg.preinsert_recovery_release_yaw_deg)
                    & (preinsert_recovery_insert_gate > 0.0)
                )
                dynamic_distance_short_bool = (
                    (
                        preinsert_recovery_dist
                        < preinsert_recovery_target_dist
                        - self.cfg.preinsert_recovery_reentry_margin_m
                    )
                    & (
                        preinsert_recovery_dist
                        < self.cfg.preinsert_recovery_reentry_dist_cap_m
                    )
                )
                preinsert_recovery_enter_bool = (
                    (
                        (
                            preinsert_recovery_trigger_dist
                            < self.cfg.preinsert_recovery_dist_trigger_m
                        )
                        | dynamic_distance_short_bool
                    )
                    & preinsert_recovery_misaligned
                ...
                preinsert_recovery_prev_latched_bool = self._preinsert_recovery_latched.clone()
                preinsert_recovery_clear_bool = preinsert_recovery_release_ready_bool
                if self.cfg.preinsert_recovery_clear_on_insert_progress:
                    preinsert_recovery_clear_bool = (
                        preinsert_recovery_clear_bool
                        | (preinsert_recovery_insert_gate <= 0.0)
                    )
                preinsert_recovery_active_bool = (
                    preinsert_recovery_prev_latched_bool | preinsert_recovery_enter_bool
                ) & (~preinsert_recovery_clear_bool)
                preinsert_recovery_safe_bool = (
                    preinsert_recovery_active_bool
                    & (preinsert_recovery_dist >= target_low)
                    & (preinsert_recovery_dist <= target_high)
                    & (preinsert_recovery_insert_gate > 0.0)
                )
                preinsert_recovery_release_bool = (
                    preinsert_recovery_prev_latched_bool
                    & preinsert_recovery_release_ready_bool
                )
                preinsert_recovery_reentry_bool = (
                    preinsert_recovery_active_bool
                    & (~preinsert_recovery_prev_latched_bool)
                    & (preinsert_recovery_dist >= self.cfg.preinsert_recovery_dist_trigger_m)
                )
                preinsert_recovery_below_target_bool = (
                    preinsert_recovery_active_bool
                    & (preinsert_recovery_dist < target_low)
                    & (preinsert_recovery_insert_gate > 0.0)
                )
                target_under = torch.clamp(
                    (target_low - preinsert_recovery_dist)
                    / max(self.cfg.preinsert_recovery_target_low_tol_m, 1e-6),
                    min=0.0,
                    max=1.0,
                )
                target_over = torch.clamp(
                    (preinsert_recovery_dist - target_high)
                    / max(self.cfg.preinsert_recovery_target_high_tol_m, 1e-6),
                    min=0.0,
                    max=1.0,
                )
                preinsert_recovery_target_band_error = (
                    preinsert_recovery_active_bool.float() * (target_under + target_over)
                )
                preinsert_recovery_dist_shortfall_error = (
                    preinsert_recovery_active_bool.float()
                    * torch.clamp(
                        (target_low - preinsert_recovery_dist)
                        / max(self.cfg.preinsert_recovery_dist_shortfall_scale_m, 1e-6),
                        min=0.0,
                        max=self.cfg.preinsert_recovery_dist_shortfall_cap,
                    )
                )
                preinsert_recovery_dist_ready_bool = (
                    preinsert_recovery_active_bool
                    & (preinsert_recovery_dist >= target_low)
                    & (preinsert_recovery_insert_gate > 0.0)
                )
                self._preinsert_recovery_was_latched[:] = (
                    preinsert_recovery_prev_latched_bool.detach()
                )
                self._preinsert_recovery_latched[:] = preinsert_recovery_active_bool.detach()
            else:
                preinsert_recovery_active_bool = (
                    (
                        preinsert_recovery_trigger_dist
                ...
                preinsert_recovery_safe_bool = (
                    (preinsert_recovery_dist >= self.cfg.preinsert_recovery_dist_target_min_m)
                    & (preinsert_recovery_dist <= self.cfg.preinsert_recovery_dist_target_max_m)
                    & preinsert_recovery_misaligned
                    & (preinsert_recovery_insert_gate > 0.0)
                )
                preinsert_recovery_target_dist = torch.full_like(
                    insert_norm,
                    0.5
                    * (
                        self.cfg.preinsert_recovery_dist_target_min_m
                        + self.cfg.preinsert_recovery_dist_target_max_m
                    ),
                )
                preinsert_recovery_below_target_bool = preinsert_recovery_active_bool
                preinsert_recovery_release_bool = torch.zeros_like(
                    insert_norm, dtype=torch.bool
                )
                preinsert_recovery_reentry_bool = torch.zeros_like(
                    insert_norm, dtype=torch.bool
                )
                preinsert_recovery_target_band_error = torch.zeros_like(insert_norm)
                preinsert_recovery_dist_shortfall_error = torch.zeros_like(insert_norm)
                preinsert_recovery_dist_ready_bool = torch.zeros_like(
                    insert_norm, dtype=torch.bool
                )
            preinsert_recovery_active = preinsert_recovery_active_bool.float()
            preinsert_recovery_safe = preinsert_recovery_safe_bool.float()
            preinsert_recovery_latched = (
                preinsert_recovery_active
                if self.cfg.preinsert_recovery_stateful_enable
                else torch.zeros_like(insert_norm)
            )
            preinsert_recovery_below_target = preinsert_recovery_below_target_bool.float()
            preinsert_recovery_release = preinsert_recovery_release_bool.float()
            preinsert_recovery_reentry = preinsert_recovery_reentry_bool.float()
            preinsert_recovery_dist_shortfall = preinsert_recovery_dist_shortfall_error
            preinsert_recovery_dist_ready = preinsert_recovery_dist_ready_bool.float()
            preinsert_recovery_narrow_mask = preinsert_recovery_narrow_mask_bool.float()
            preinsert_recovery_action_shortfall_gate = preinsert_recovery_below_target
            if self.cfg.preinsert_recovery_action_shortfall_gate_enable:
                preinsert_recovery_action_shortfall_gate = preinsert_recovery_active * torch.clamp(
                    self.cfg.preinsert_recovery_action_shortfall_floor
                    + (
                        1.0
                        - self.cfg.preinsert_recovery_action_shortfall_floor
                    )
                    * preinsert_recovery_dist_shortfall,
                    min=0.0,
                    max=1.0,
                )
            preinsert_recovery_gate = torch.where(
                preinsert_recovery_active_bool,
                torch.zeros_like(insert_norm),
                torch.ones_like(insert_norm),
            )
            if self.cfg.preinsert_recovery_gate_positive_rewards:
                r_d = r_d * preinsert_recovery_gate
                r_cd = r_cd * preinsert_recovery_gate
                r_cpsi = r_cpsi * preinsert_recovery_gate
        else:
            preinsert_recovery_active = torch.zeros_like(insert_norm)
            preinsert_recovery_safe = torch.zeros_like(insert_norm)
            preinsert_recovery_latched = torch.zeros_like(insert_norm)
            preinsert_recovery_target_dist = torch.zeros_like(insert_norm)
            preinsert_recovery_below_target = torch.zeros_like(insert_norm)
            preinsert_recovery_release = torch.zeros_like(insert_norm)
            preinsert_recovery_reentry = torch.zeros_like(insert_norm)
            preinsert_recovery_target_band_error = torch.zeros_like(insert_norm)
            preinsert_recovery_dist_shortfall = torch.zeros_like(insert_norm)
            preinsert_recovery_dist_ready = torch.zeros_like(insert_norm)
            preinsert_recovery_action_shortfall_gate = torch.zeros_like(insert_norm)
            preinsert_recovery_gate = torch.ones_like(insert_norm)
            preinsert_recovery_narrow_mask_bool = torch.zeros_like(insert_norm, dtype=torch.bool)
            preinsert_recovery_narrow_mask = torch.zeros_like(insert_norm)

        if self.cfg.preinsert_recovery_enable and self.cfg.preinsert_recovery_stateful_enable:
            r_preinsert_recovery_target_band = -(
                self.cfg.preinsert_recovery_target_band_weight
                * preinsert_recovery_target_band_error
            )
            r_preinsert_recovery_dist_shortfall = -(
                self.cfg.preinsert_recovery_dist_shortfall_weight
                * preinsert_recovery_dist_shortfall
            )
            r_preinsert_recovery_dist_ready = (
                self.cfg.preinsert_recovery_dist_ready_bonus_weight
                * preinsert_recovery_dist_ready
            )
            r_preinsert_recovery_release_bonus = (
                self.cfg.preinsert_recovery_release_bonus_weight
                * preinsert_recovery_release
            )
            r_preinsert_recovery_terminal_penalty = -(
                self.cfg.preinsert_recovery_terminal_penalty_weight
                * training_success_next.float()
                * preinsert_recovery_active
            )
        else:
            r_preinsert_recovery_target_band = torch.zeros_like(insert_norm)
            r_preinsert_recovery_dist_shortfall = torch.zeros_like(insert_norm)
            r_preinsert_recovery_dist_ready = torch.zeros_like(insert_norm)
            r_preinsert_recovery_release_bonus = torch.zeros_like(insert_norm)
            r_preinsert_recovery_terminal_penalty = torch.zeros_like(insert_norm)
        ...
        if self.cfg.dirty_insert_early_penalty_enable:
            dirty_insert_early_progress = smoothstep(
                (insert_norm - self.cfg.dirty_insert_early_start_frac)
                / max(self.cfg.dirty_insert_early_ramp_frac, 1e-6)
            )
            dirty_insert_early_push = smoothstep(
                (pallet_disp_xy - self.cfg.dirty_insert_early_push_start_m)
                / max(self.cfg.dirty_insert_early_push_ramp_m, 1e-6)
            )
            if self.cfg.dirty_insert_early_use_progress_delta:
                dirty_insert_early_scale = torch.clamp(
                    insert_norm_delta
                    / max(self.cfg.dirty_insert_early_progress_delta_scale_frac, 1e-6),
                    min=0.0,
                    max=1.0,
                )
            else:
                dirty_insert_early_scale = dirty_insert_early_progress
            dirty_insert_early_gate = (
                1.0
                - dirty_insert_early_progress * dirty_insert_early_push
                + dirty_insert_early_progress
                * dirty_insert_early_push
                * self.cfg.dirty_insert_early_gate_floor
            )
            r_dirty_insert_early = -(
                self.cfg.dirty_insert_early_penalty_weight
                * dirty_insert_early_scale
                * dirty_insert_early_push
            )
        else:
            dirty_insert_early_progress = torch.zeros_like(insert_norm)
            dirty_insert_early_push = torch.zeros_like(insert_norm)
            dirty_insert_early_gate = torch.ones_like(insert_norm)
            r_dirty_insert_early = torch.zeros_like(insert_norm)

        if self.cfg.dirty_insert_early_gate_positive_rewards:
            r_d = r_d * dirty_insert_early_gate
            r_cd = r_cd * dirty_insert_early_gate
            r_cpsi = r_cpsi * dirty_insert_early_gate

        if self.cfg.eval_dirty_insert_penalty_enable:
            eval_dirty_insert_progress = smoothstep(
                (insert_norm - self.cfg.eval_dirty_insert_progress_start_frac)
                / max(self.cfg.eval_dirty_insert_progress_ramp_frac, 1e-6)
            )
            eval_dirty_insert_gate = (
                1.0
                - eval_dirty_insert_progress * self._ever_dirty_insert_eval.float()
                + eval_dirty_insert_progress
                * self._ever_dirty_insert_eval.float()
                * self.cfg.eval_dirty_insert_gate_floor
            )
            r_eval_dirty_insert_once = -(
                self.cfg.eval_dirty_insert_once_penalty_weight
                * eval_dirty_insert_new.float()
            )
            r_eval_dirty_insert_persistent = -(
                self.cfg.eval_dirty_insert_persistent_penalty_weight
                * self._ever_dirty_insert_eval.float()
                * eval_dirty_insert_progress
            )
        else:
            eval_dirty_insert_progress = torch.zeros_like(insert_norm)
            eval_dirty_insert_gate = torch.ones_like(insert_norm)
            r_eval_dirty_insert_once = torch.zeros_like(insert_norm)
            r_eval_dirty_insert_persistent = torch.zeros_like(insert_norm)

        if self.cfg.eval_dirty_insert_gate_positive_rewards:
            r_d = r_d * eval_dirty_insert_gate
            r_cd = r_cd * eval_dirty_insert_gate
            r_cpsi = r_cpsi * eval_dirty_insert_gate

        if self.cfg.eval_dirty_preinsert_penalty_enable:
            eval_dirty_preinsert_contact = smoothstep(
                (
                    self.cfg.eval_dirty_preinsert_contact_dist_m
                    + self.cfg.eval_dirty_preinsert_contact_ramp_m
                    - dist_front
                )
                / max(self.cfg.eval_dirty_preinsert_contact_ramp_m, 1e-6)
            )
            eval_dirty_preinsert_insert_gate = (
                insert_norm < self.cfg.eval_dirty_preinsert_insert_frac_max
            ).float()
            eval_dirty_preinsert_active = (
                eval_dirty_preinsert_contact * eval_dirty_preinsert_insert_gate
            )
            eval_dirty_preinsert_push = torch.clamp(
                (
                    self._max_pallet_disp_xy_eval
                    - self.cfg.push_free_disp_thresh_m
                    - self.cfg.eval_dirty_insert_disp_margin_m
                )
                / max(self.cfg.eval_dirty_preinsert_disp_scale_m, 1e-6),
                min=0.0,
                max=1.0,
            )
            eval_dirty_preinsert_gate = (
                1.0
                - eval_dirty_preinsert_active * eval_dirty_preinsert_push
                + eval_dirty_preinsert_active
                * eval_dirty_preinsert_push
                * self.cfg.eval_dirty_preinsert_gate_floor
            )
            r_eval_dirty_preinsert = -(
                self.cfg.eval_dirty_preinsert_penalty_weight
                * eval_dirty_preinsert_active
                * eval_dirty_preinsert_push
            )
        else:
            eval_dirty_preinsert_active = torch.zeros_like(insert_norm)
            eval_dirty_preinsert_push = torch.zeros_like(insert_norm)
            eval_dirty_preinsert_gate = torch.ones_like(insert_norm)
            r_eval_dirty_preinsert = torch.zeros_like(insert_norm)

        if self.cfg.eval_dirty_preinsert_gate_positive_rewards:
            r_d = r_d * eval_dirty_preinsert_gate
            r_cd = r_cd * eval_dirty_preinsert_gate
            r_cpsi = r_cpsi * eval_dirty_preinsert_gate
        ...
        if self.cfg.clean_insert_push_free_bonus_enable:
            ...
            if self.cfg.preinsert_recovery_gate_positive_rewards:
                r_clean_insert_bonus = r_clean_insert_bonus * preinsert_recovery_gate
        else:
        ...
        if self.cfg.preinsert_align_reward_enable:
            ...
            prev_recovery_dist = self._prev_dist_front
            if self.cfg.preinsert_recovery_dist_ref == "fork_center":
                prev_recovery_dist = self._prev_dist_front + 0.6
            elif self.cfg.preinsert_recovery_dist_ref == "root":
                prev_recovery_dist = self._prev_dist_front + float(self._fork_forward_offset)
            ...
            recovery_dist_delta = torch.clamp(
                prev_recovery_dist - preinsert_recovery_dist,
                min=-self.cfg.preinsert_delta_clip_dist_m,
                max=self.cfg.preinsert_delta_clip_dist_m,
            )
            ...
            recovery_dist_delta_pos = torch.clamp(
                recovery_dist_delta / max(self.cfg.preinsert_delta_clip_dist_m, 1e-6),
                min=0.0,
                max=1.0,
            )
            recovery_retreat_delta = torch.clamp(
                -recovery_dist_delta / max(self.cfg.preinsert_delta_clip_dist_m, 1e-6),
                min=0.0,
                max=1.0,
            )
            ...
            if (
                self.cfg.preinsert_recovery_gate_positive_rewards
                and self.cfg.preinsert_recovery_stateful_enable
            ):
                r_preinsert_align = r_preinsert_align * preinsert_recovery_gate
            ...
            preinsert_recovery_retreat_mask = (
                ...
                if self.cfg.preinsert_recovery_stateful_enable
                else preinsert_recovery_active
            )
            r_preinsert_recovery_retreat = (
                self.cfg.preinsert_recovery_retreat_reward_weight
                * preinsert_recovery_retreat_mask
            ...
            reverse_steer_need = torch.maximum(
                torch.clamp(
                    yaw_err_deg
                    / max(self.cfg.preinsert_recovery_reverse_steer_align_yaw_scale_deg, 1e-6),
                    min=0.0,
                    max=1.0,
                ),
                torch.maximum(
                    torch.clamp(
                        center_y_err
                        / max(self.cfg.preinsert_recovery_reverse_steer_align_center_scale_m, 1e-6),
                        min=0.0,
                        max=1.0,
                    ),
                    torch.clamp(
                        tip_y_err
                        / max(self.cfg.preinsert_recovery_reverse_steer_align_tip_scale_m, 1e-6),
                        min=0.0,
                        max=1.0,
                    ),
                ),
            )
            reverse_steer_improve = torch.clamp(
                0.5 * (y_delta_pos + yaw_delta_pos),
                min=0.0,
                max=1.0,
            )
            reverse_steer_improve = torch.maximum(
                reverse_steer_improve,
                torch.full_like(
                    reverse_steer_improve,
                    self.cfg.preinsert_recovery_reverse_steer_align_improve_floor,
                ),
            )
            reverse_steer_align_gate = (
                preinsert_recovery_retreat_mask
                * (
                    yaw_err_deg
                    >= self.cfg.preinsert_recovery_reverse_steer_align_min_yaw_deg
                ).float()
            )
            reverse_steer_align_score = (
                reverse_steer_align_gate
                * recovery_reverse_action
                * recovery_steer_action
                * reverse_steer_need
                * reverse_steer_improve
            )
            r_preinsert_recovery_reverse_steer_align = (
                self.cfg.preinsert_recovery_reverse_steer_align_weight
                * reverse_steer_align_score
            )
            r_preinsert_recovery_safe_align = (
                self.cfg.preinsert_recovery_safe_align_weight
                * preinsert_recovery_safe
                * (y_delta_pos + yaw_delta_pos)
            )
            r_preinsert_recovery_forward_block = -(
                self.cfg.preinsert_recovery_forward_block_weight
                * preinsert_recovery_active
                * recovery_dist_delta_pos
            )
            r_preinsert_recovery_forward_action = -(
                self.cfg.preinsert_recovery_forward_action_penalty_weight
                * preinsert_recovery_retreat_mask
            ...
            recovery_dist_delta = torch.zeros_like(dist_front)
            recovery_reverse_action = torch.zeros_like(insert_norm)
            recovery_forward_action = torch.zeros_like(insert_norm)
            recovery_steer_action = torch.zeros_like(insert_norm)
            reverse_steer_align_score = torch.zeros_like(insert_norm)
            preinsert_forward_gate = torch.ones_like(insert_norm)
            preinsert_forward_gate_core = torch.ones_like(insert_norm)
            ...
            r_preinsert_recovery_reverse_action = torch.zeros_like(insert_norm)
            r_preinsert_recovery_reverse_steer_align = torch.zeros_like(insert_norm)
        ...
        if self.cfg.clean_insert_progress_reward_enable:
            ...
            if self.cfg.preinsert_recovery_gate_positive_rewards:
                r_clean_insert_progress = r_clean_insert_progress * preinsert_recovery_gate
        else:
        ...
        if self.cfg.push_free_insert_progress_reward_enable:
            push_free_insert_progress_active = (
                (insert_norm >= self.cfg.push_free_insert_progress_start_frac)
                & (insert_norm <= self.cfg.push_free_insert_progress_end_frac)
            ).float()
            r_push_free_insert_progress = (
                self.cfg.push_free_insert_progress_reward_weight
                * push_free_insert_progress_active
                * push_free.float()
                * insert_progress_norm
            )
            if self.cfg.preinsert_recovery_gate_positive_rewards:
                r_push_free_insert_progress = (
                    r_push_free_insert_progress * preinsert_recovery_gate
                )
        else:
            push_free_insert_progress_active = torch.zeros_like(insert_norm)
            r_push_free_insert_progress = torch.zeros_like(insert_norm)
        ...
        if self.cfg.visual_envelope_termination_enable:
            visual_envelope_termination = self._visual_envelope_termination
            r_visual_envelope_termination = -(
                self.cfg.visual_envelope_termination_penalty_weight
                * visual_envelope_termination.float()
            )
        else:
            visual_envelope_termination = torch.zeros_like(success_next, dtype=torch.bool)
            r_visual_envelope_termination = torch.zeros_like(insert_norm)
        ...
        if self.cfg.push_free_rg_gate_enable:
            rg = (success_geom_training & push_free_train).float()
        else:
        ...
        if (
            self.cfg.preinsert_recovery_gate_success_done
            and self.cfg.preinsert_recovery_stateful_enable
        ):
            success_next = success_next & (preinsert_recovery_gate > 0.0)

        success_reward_gate = torch.ones_like(insert_norm)
        if self.cfg.preinsert_recovery_gate_success_rewards:
            success_reward_gate = preinsert_recovery_gate
        r_success = success_next.float() * success_reward_gate * self.cfg.rew_success
        r_success_time = success_next.float() * success_reward_gate * self.cfg.rew_success_time
        ...
        push_free_success_strict = success_strict & push_free_train
        ...
        inserted_push_free = is_inserted & push_free_train
        clean_insert_ready = is_inserted & is_center_aligned_cfg & is_tip_constraint_ok & push_free_train
        dirty_insert = is_inserted & ~push_free_train
        ...
        self.extras["log"]["paper_reward/r_dirty_insert_early"] = r_dirty_insert_early.mean()
        self.extras["log"]["paper_reward/r_eval_dirty_insert_once"] = r_eval_dirty_insert_once.mean()
        self.extras["log"]["paper_reward/r_eval_dirty_insert_persistent"] = r_eval_dirty_insert_persistent.mean()
        self.extras["log"]["paper_reward/r_eval_dirty_preinsert"] = (
            r_eval_dirty_preinsert.mean()
        )
        ...
        self.extras["log"]["paper_reward/r_push_free_insert_progress"] = (
        ...
        self.extras["log"]["paper_reward/r_preinsert_recovery_retreat"] = (
            r_preinsert_recovery_retreat.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_reverse_action"] = (
            r_preinsert_recovery_reverse_action.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_reverse_steer_align"] = (
            r_preinsert_recovery_reverse_steer_align.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_safe_align"] = (
            r_preinsert_recovery_safe_align.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_forward_block"] = (
            r_preinsert_recovery_forward_block.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_forward_action"] = (
            r_preinsert_recovery_forward_action.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_target_band"] = (
            r_preinsert_recovery_target_band.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_dist_shortfall"] = (
            r_preinsert_recovery_dist_shortfall.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_dist_ready"] = (
            r_preinsert_recovery_dist_ready.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_release_bonus"] = (
            r_preinsert_recovery_release_bonus.mean()
        )
        self.extras["log"]["paper_reward/r_preinsert_recovery_terminal_penalty"] = (
            r_preinsert_recovery_terminal_penalty.mean()
        )
        ...
        self.extras["log"]["paper_reward/r_visual_envelope_termination"] = (
            r_visual_envelope_termination.mean()
        )
        ...
        # 核心诊断指标
        self.extras["log"]["err/dist_front_mean"] = dist_front.mean()
        self.extras["log"]["err/dist_front_center_mean"] = dist_front_center.mean()
        self.extras["log"]["err/dist_front_root_mean"] = dist_front_root.mean()
        self.extras["log"]["err/preinsert_recovery_dist_mean"] = preinsert_recovery_dist.mean()
        self.extras["log"]["err/preinsert_recovery_trigger_dist_mean"] = (
            preinsert_recovery_trigger_dist.mean()
        )
        self.extras["log"]["err/lateral_mean"] = y_err.mean()
        ...
        self.extras["log"]["diag/preinsert_recovery_safe_frac"] = (
            preinsert_recovery_safe.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_gate_mean"] = (
            preinsert_recovery_gate.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_latched_frac"] = (
            preinsert_recovery_latched.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_target_dist_mean"] = (
            preinsert_recovery_target_dist.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_below_target_frac"] = (
            preinsert_recovery_below_target.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_dist_shortfall_mean"] = (
            preinsert_recovery_dist_shortfall.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_action_shortfall_gate_mean"] = (
            preinsert_recovery_action_shortfall_gate.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_narrow_mask_frac"] = (
            preinsert_recovery_narrow_mask.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_dist_ready_frac"] = (
            preinsert_recovery_dist_ready.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_release_frac"] = (
            preinsert_recovery_release.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_reentry_frac"] = (
            preinsert_recovery_reentry.mean()
        )
        self.extras["log"]["diag/preinsert_recovery_success_reward_gate_mean"] = (
            success_reward_gate.mean()
        )
        self.extras["log"]["diag/near_hard_curriculum_frac"] = (
            self._debug_near_hard_curriculum.float().mean()
        )
        ...
        self.extras["log"]["diag/dirty_insert_early_progress_mean"] = (
            dirty_insert_early_progress.mean()
        )
        self.extras["log"]["diag/dirty_insert_early_push_mean"] = (
            dirty_insert_early_push.mean()
        )
        self.extras["log"]["diag/dirty_insert_early_gate_mean"] = (
            dirty_insert_early_gate.mean()
        )
        self.extras["log"]["diag/eval_dirty_insert_progress_mean"] = (
            eval_dirty_insert_progress.mean()
        )
        self.extras["log"]["diag/eval_dirty_insert_gate_mean"] = (
            eval_dirty_insert_gate.mean()
        )
        self.extras["log"]["diag/eval_dirty_insert_new_frac"] = (
            eval_dirty_insert_new.float().mean()
        )
        self.extras["log"]["diag/eval_dirty_insert_ever_frac"] = (
            self._ever_dirty_insert_eval.float().mean()
        )
        self.extras["log"]["diag/eval_max_pallet_disp_xy_mean"] = (
            self._max_pallet_disp_xy_eval.mean()
        )
        self.extras["log"]["diag/eval_dirty_max_gate_mean"] = eval_dirty_max_gate.mean()
        self.extras["log"]["diag/eval_dirty_preinsert_active_frac"] = (
            eval_dirty_preinsert_active.mean()
        )
        self.extras["log"]["diag/eval_dirty_preinsert_push_mean"] = (
            eval_dirty_preinsert_push.mean()
        )
        self.extras["log"]["diag/eval_dirty_preinsert_gate_mean"] = (
            eval_dirty_preinsert_gate.mean()
        )
        self.extras["log"]["diag/push_free_insert_progress_active_frac"] = (
            push_free_insert_progress_active.mean()
        )
        ...
        self.extras["log"]["diag/visual_envelope_termination_frac"] = (
            visual_envelope_termination.float().mean()
        )
        ...
        self.extras["log"]["diag/preinsert_recovery_dist_delta_mean"] = recovery_dist_delta.mean()
        self.extras["log"]["diag/preinsert_recovery_reverse_action_mean"] = recovery_reverse_action.mean()
        self.extras["log"]["diag/preinsert_recovery_forward_action_mean"] = recovery_forward_action.mean()
        self.extras["log"]["diag/preinsert_recovery_steer_action_mean"] = recovery_steer_action.mean()
        self.extras["log"]["diag/preinsert_recovery_reverse_steer_score_mean"] = (
            reverse_steer_align_score.mean()
        )
        self.extras["log"]["diag/preinsert_action_guard_active_frac"] = (
            self._preinsert_action_guard_active.float().mean()
        )
        self.extras["log"]["diag/preinsert_action_guard_applied_frac"] = (
            self._preinsert_action_guard_applied.float().mean()
        )
        self.extras["log"]["diag/preinsert_action_guard_drive_delta_mean"] = (
            self._preinsert_action_guard_drive_delta.mean()
        )
        self.extras["log"]["diag/preinsert_action_guard_used_frac"] = (
            self._preinsert_action_guard_used.float().mean()
        )
        self.extras["log"]["diag/preinsert_action_guard_steps_mean"] = (
            self._preinsert_action_guard_steps.float().mean()
        )
    ...
    def _visual_envelope_termination_mask(self) -> torch.Tensor:
        """Early-stop poses that leave the default Toyota side-camera visibility envelope."""
        if not bool(getattr(self.cfg, "visual_envelope_termination_enable", False)):
            return torch.zeros((self.num_envs,), dtype=torch.bool, device=self.device)
        ...
        return (
            (self.episode_length_buf >= int(getattr(self.cfg, "visual_envelope_min_steps", 8)))
            & (insert_norm <= float(getattr(self.cfg, "visual_envelope_insert_norm_max", 0.35)))
            & (
                (center_y_err >= float(getattr(self.cfg, "visual_envelope_center_y_m", 0.80)))
                | (tip_y_err >= float(getattr(self.cfg, "visual_envelope_tip_y_m", 0.90)))
                | (yaw_err_deg >= float(getattr(self.cfg, "visual_envelope_yaw_deg", 18.0)))
            )
            & (~self._success_termination)
        )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self._visual_envelope_termination = self._visual_envelope_termination_mask()
        terminated = (
            tipped
            | success
            | out_of_bounds
            ...
            | self._visual_envelope_termination
            | self._progress_teacher_dirty_insert_termination
        )
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._visual_envelope_termination[env_ids] = False
        ...
        self._prev_center_y_err[env_ids] = 0.0
        self._prev_tip_y_err[env_ids] = 0.0
        self._prev_tip_axis[env_ids] = 0.0
        ...
        self._prev_stage_dist_front[env_ids] = 0.0
        # S1.0Q: 死区撤退 / 横向精调状态量清零
        self._prev_insert_norm[env_ids] = 0.0
        self._prev_in_dead_zone[env_ids] = False
        self._ever_inserted_eval[env_ids] = False
        self._ever_dirty_insert_eval[env_ids] = False
        self._max_pallet_disp_xy_eval[env_ids] = 0.0
        self._prev_phi_lat[env_ids] = 0.0
        self._preinsert_recovery_latched[env_ids] = False
        self._preinsert_recovery_was_latched[env_ids] = False
        self._preinsert_action_guard_latched[env_ids] = False
        self._preinsert_action_guard_active[env_ids] = False
        self._preinsert_action_guard_applied[env_ids] = False
        self._preinsert_action_guard_drive_delta[env_ids] = 0.0
        ...
        # ---- 托盘固定位姿（可选：后续可加随机化） ----
        # Root pose writers expect simulation/world-frame coordinates.  The reset
        # randomization below is expressed in each cloned env's local frame.
        ...
        # ---- 随机化叉车初始位姿 ----
        if self._stage_1_mode:
            if bool(getattr(self.cfg, "stage1_use_triangular_visible_init", False)):
                tri_x_near = float(self.cfg.stage1_tri_x_near_m)
                tri_x_far = float(self.cfg.stage1_tri_x_far_m)
                t = torch.rand((len(env_ids), 1), device=self.device)
                x = tri_x_near + (tri_x_far - tri_x_near) * t
                y_half = (
                    float(self.cfg.stage1_tri_y_half_width_near_m)
                    + (
                        float(self.cfg.stage1_tri_y_half_width_far_m)
                        - float(self.cfg.stage1_tri_y_half_width_near_m)
                    )
                    * t
                )
                yaw_half_deg = (
                    float(self.cfg.stage1_tri_yaw_deg_near)
                    + (float(self.cfg.stage1_tri_yaw_deg_far) - float(self.cfg.stage1_tri_yaw_deg_near)) * t
                )
                y = (2.0 * torch.rand((len(env_ids), 1), device=self.device) - 1.0) * y_half
                yaw = (
            ...
            if bool(self.cfg.stage1_near_hard_curriculum_enable):
                ...
                ramp = max(
                    float(getattr(self.cfg, "stage1_near_hard_curriculum_ramp_steps", 1.0)),
                    1.0,
                )
                ...
                if near_hard_count > 0:
                    hard_y_frac = max(
                        0.0,
                        min(1.0, float(getattr(self.cfg, "stage1_near_hard_lateral_frac", 0.5))),
                    )
                    hard_y_mask = torch.rand((len(env_ids), 1), device=self.device) < hard_y_frac
                    y_abs = sample_uniform(
                        self.cfg.stage1_near_hard_y_abs_min_m,
                        self.cfg.stage1_near_hard_y_abs_max_m,
                        (len(env_ids), 1),
                        device=self.device,
                    )
                    positive_y_frac = max(
                        0.0,
                        min(1.0, float(getattr(self.cfg, "stage1_near_hard_positive_y_frac", 0.5))),
                    )
                    y_sign = torch.where(
                        torch.rand((len(env_ids), 1), device=self.device) < positive_y_frac,
                        torch.ones((len(env_ids), 1), device=self.device),
                        -torch.ones((len(env_ids), 1), device=self.device),
                    )
                    yaw_abs = sample_uniform(
                        self.cfg.stage1_near_hard_yaw_abs_min_deg * math.pi / 180.0,
                        self.cfg.stage1_near_hard_yaw_abs_max_deg * math.pi / 180.0,
                        (len(env_ids), 1),
                        device=self.device,
                    )
                    y_base = sample_uniform(
                        self.cfg.stage1_init_y_min_m,
                        self.cfg.stage1_init_y_max_m,
                        (len(env_ids), 1),
                        device=self.device,
                    )
                    yaw_base = sample_uniform(
                        self.cfg.stage1_init_yaw_deg_min * math.pi / 180.0,
                        self.cfg.stage1_init_yaw_deg_max * math.pi / 180.0,
                        (len(env_ids), 1),
                        device=self.device,
                    ...
                    yaw_sign = torch.where(
                        ...
                        < max(
                            0.0,
                            min(1.0, float(getattr(self.cfg, "stage1_near_hard_positive_yaw_frac", 0.5))),
                        ),
                        torch.ones((len(env_ids), 1), device=self.device),
                        -torch.ones((len(env_ids), 1), device=self.device),
                    )
                    opposite_yaw_frac = max(
                        0.0,
                        min(1.0, float(getattr(self.cfg, "stage1_near_hard_opposite_yaw_frac", 0.0))),
                    )
                    opposite_yaw_mask = (
                        torch.rand((len(env_ids), 1), device=self.device) < opposite_yaw_frac
                    )
                    yaw_sign = torch.where(opposite_yaw_mask, -y_sign, yaw_sign)
                    x_hard = sample_uniform(
                        self.cfg.stage1_near_hard_x_min_m,
                        self.cfg.stage1_near_hard_x_max_m,
                        (len(env_ids), 1),
                        device=self.device,
                    )
                    y_hard = torch.where(hard_y_mask, y_abs * y_sign, y_base)
                    yaw_hard = torch.where(hard_y_mask, yaw_base, yaw_abs * yaw_sign)
                    x = torch.where(near_hard_mask, x_hard, x)
                    y = torch.where(near_hard_mask, y_hard, y)
                    yaw = torch.where(near_hard_mask, yaw_hard, yaw)
                    self._debug_near_hard_curriculum[env_ids] = near_hard_mask.squeeze(-1)
        else:
        ...
        self._debug_reset_y[env_ids] = y.squeeze(-1)
        ...
        center_x_reset = tip_x_reset - 0.6 * cos_yaw_reset
        center_y_reset = tip_y_reset - 0.6 * sin_yaw_reset
        rel_center_reset = torch.stack([center_x_reset, center_y_reset], dim=-1) - pallet_pos_reset
        tip_y_err_reset = torch.abs(torch.sum(rel_tip_reset * torch.stack([-sp_reset, cp_reset], dim=-1), dim=-1))
        center_y_err_reset = torch.abs(torch.sum(rel_center_reset * torch.stack([-sp_reset, cp_reset], dim=-1), dim=-1))
        ...
        elif self.cfg.stage_distance_ref == "base":
        ...
        self._prev_stage_dist_front[env_ids] = dist_front_reset.detach()
        self._prev_y_err[env_ids] = y_err_reset.detach()
        self._prev_center_y_err[env_ids] = center_y_err_reset.detach()
        self._prev_tip_y_err[env_ids] = tip_y_err_reset.detach()
        self._prev_tip_axis[env_ids] = s_tip_reset.detach()
