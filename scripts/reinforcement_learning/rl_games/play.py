...
# Allowlist numpy scalar for torch.load in PyTorch 2.6+ (weights_only default).
if hasattr(torch.serialization, "add_safe_globals"):
    safe_globals = [np.core.multiarray.scalar, np.dtype]
    if hasattr(np, "dtypes") and hasattr(np.dtypes, "Float32DType"):
        safe_globals.append(np.dtypes.Float32DType)
